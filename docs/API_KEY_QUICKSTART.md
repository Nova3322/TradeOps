...
打开 **右上角用户菜单 → API Key** 后，页面顶部会直接显示“创建 API Key”表单；
不需要再进入其他文档页或展开隐藏面板。本页同时内嵌快速开始、调用示例、安全边界
和常用接口目录，完整字段仍以运行中的 OpenAPI 为准。明文只在创建或轮换成功时显示
一次，应存入秘密管理器，不得写入源码、Prompt、截图、聊天或普通日志。
