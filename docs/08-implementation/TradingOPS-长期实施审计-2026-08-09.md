...
- `0031` 用户任务导航收敛：保留 24 条既有 SPA 页面及全部服务端 capability、团队/账户隔离、创建者自审拒绝、幂等、版本快照、审计、风控和环境隔离，只把一级导航从 13 个系统模块收敛为“今日、信号、提案、交易、复盘”五个用户任务。右上角新增单一“团队设置”，按当前角色渐进展示访问控制、信号源、交易账户、风控、通知、影子/真实模式、AI Agent、资金与诊断；未授权卡片不渲染。提案内部统一“我的提案、待我审核、全部记录、手动创建”，交易内部统一任务、仓位与保护、订单与成交、异常；信号先展示当前事实，配置默认折叠。首次就绪以创建团队、接入信号与账户、确认风控与通知、完成影子提案闭环四步呈现；只为 `/settings` 增加同一 `index.html` 的 GET 壳以支持硬刷新，不新增实体、迁移、业务 API 或第二套状态真源。
- `0031` 五维验收：数据库迁移不适用且 Schema 保持 `20260810_0025`，业务 API 合同不变；`/settings` 硬刷新实际返回 `200`。`node --check`、mypy 和 `git diff --check` 通过，完整单元/API `446 passed`，NoTilt 13 个和 Safe Spending 5 个 Node 合同测试通过。真实本地 PostgreSQL、dry-run workers 和密码会话在 `127.0.0.1:8014` 运行；浏览器逐页验证五个一级任务、团队设置四步、提案/交易标签、active navigation 和真实服务端数据。1440×900、1024×768、430×932、390×844 四个 CSS 视口共 24 个页面检查均无文档或顶栏横向溢出、错误页或导航数量偏差；390 px 下 Enter 打开菜单并把焦点移到“今日”，Escape 关闭并恢复到菜单按钮，可见焦点轮廓与 `aria-expanded` 同步。深浅主题切换和硬刷新后的 `aria-pressed`、标签及页面层级一致，浏览器错误/警告为 0；截图为 `/private/tmp/tradingops-nav-390-dark.png`、`/private/tmp/tradingops-nav-390-light.png`、`/private/tmp/tradingops-settings-1440-light.png`、`/private/tmp/tradingops-settings-1440-dark.png`。本阶段没有真实下单、资金划转、签名或广播；最终浏览器进程关闭了 runtime sync、Perptape WebSocket 与 Telegram，外部限流不会影响本地 UI 真源验收。
