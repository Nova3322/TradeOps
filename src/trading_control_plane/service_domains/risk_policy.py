...
class PolicyRiskService(ServiceComponent):
    ...
    def _managed_capital_context(
        ...
        *,
        ...
        if environment == ExecutionEnvironment.SHADOW.value:
            ...
            if team is not None and team.execution_mode_locked_at is not None:
                if account is None:
                    ...
                    [
                        {
                            ...
                            "usd_value": str(account.equity),
                            "available_balance": str(account.available_balance),
                            "updated_at": account.updated_at.isoformat(),
                        },
                        {
                            "capital_transfer_reserved_usd": "0",
                            "managed_capital_usd": str(account.equity),
                        },
                    ],
                    now,
                )
    ...
    def _server_risk_context(
        ...
        *,
        ...
        if (
            ...
            return self._shadow_server_risk_context(
                session,
                proposal=proposal,
                policy=policy,
                kind=kind,
    ...
    def _shadow_server_risk_context(
        self,
        session: Session,
        *,
        proposal: Proposal,
        ...
        shadow_instrument = session.scalar(
            select(ShadowInstrument).where(
                ...
                ShadowInstrument.catalog_instrument_id == proposal.instrument_id,
            )
        )
        position = None
        if account is not None and shadow_instrument is not None:
            position = session.scalar(
        ...
        if position is not None:
            ...
            protection = session.scalar(
                ...
                .where(
                    ...
                    ShadowOrder.shadow_position_id == position.shadow_position_id,
        ...
        position_known = account is not None
        protection_required = kind is IntentKind.ADD or (
        ...
        inputs = RiskEvaluationInput(
            ...
            account_consecutive_losses=account_loss_streak,
            loss_cooldown_remaining=loss_cooldown_remaining,
            fact_age=timedelta(0),
            position_known=position_known,
            equity_known=capital_known,
            protection_known=protection_known,
            source_current=True,
        )
        facts = {
            ...
            "position": (
                {
                    ...
                    "quantity": "0",
                    "fact_status": "KNOWN" if account is not None else "UNKNOWN",
                }
                if position is None
                else {
            ...
            if account is None
                ...
                "available_balance": str(account.available_balance),
                "fact_status": "KNOWN",
                "written_at": account.updated_at.isoformat(),
            },
            "managed_capital": {
                "known": capital_known,
                "total_usd": str(managed_capital_usd),
                "effective_max_total_risk": str(
                    min(policy.max_total_risk, managed_capital_usd)
                ),
                "facts": managed_facts,
            },
            ...
            if protection is None
                ...
                "shadow_order_id": str(protection.shadow_order_id),
                "status": protection.status,
                "quantity": str(protection.quantity),
                "fully_covered": True,
                "written_at": protection.updated_at.isoformat(),
            },
            "data_as_of": capital_as_of.isoformat(),
            "fact_age_seconds": "0",
            "generation": None if account is None else account.generation,
        }
        return (
