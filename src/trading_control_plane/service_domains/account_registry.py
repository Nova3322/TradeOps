    ...
    with self.database.session_factory.begin() as session:
        ...
        if active_campaign is not None:
            _reject(
                ...
                "close every active LIVE campaign for this exact account before deletion",
            )
        pending_transfer = session.scalar(
            select(CapitalTransfer.capital_transfer_id)
            .where(
                CapitalTransfer.team_id == team.team_id,
                CapitalTransfer.account_id == account.account_id,
                CapitalTransfer.venue == account.venue,
        ...
        pending_direct = session.scalar(
            select(DirectCapitalOperation.operation_id)
            .where(
                DirectCapitalOperation.team_id == team.team_id,
                DirectCapitalOperation.account_id == account.account_id,
                DirectCapitalOperation.venue == account.venue,
                DirectCapitalOperation.status.not_in({"BLOCKED", "SETTLED"}),
            )
            .limit(1)
        )
        if pending_transfer is not None or pending_direct is not None:
            _reject(
                "EXCHANGE_ACCOUNT_DELETE_BLOCKED",
                "resolve every in-flight or unknown capital operation before deletion",
            )
