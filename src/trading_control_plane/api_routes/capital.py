...
def register_capital_routes(context: ApiRouteContext) -> None:
    """Register capital routes against one application dependency context."""

    app = context.app
    capital_snapshot = context.require("capital_snapshot")
    configured_notilt_scope = context.require("configured_notilt_scope")
    effective_direct_capital_settings = context.require("effective_direct_capital_settings")
    identity_dependency = context.require("identity_dependency")
    notify_capital = context.require("notify_capital")
    notilt_chain_id_for_network = context.require("notilt_chain_id_for_network")
    queries = context.require("queries")
    resolved_binance_capital = context.require("resolved_binance_capital")
    resolved_capital_transfer = context.require("resolved_capital_transfer")
    resolved_hyperliquid_capital = context.require("resolved_hyperliquid_capital")
    resolved_notilt = context.require("resolved_notilt")
    resolved_safe_spending = context.require("resolved_safe_spending")
    resolved_settings = context.require("resolved_settings")
    service = context.require("service")
    sync_configured_notilt_vault = context.require("sync_configured_notilt_vault")
    token_service = context.require("token_service")
    verify_live_notilt_release_budget = context.require("verify_live_notilt_release_budget")
    ...
    def create_transfer_proposal(
        ...
        return cast(
            dict[str, Any], queries().transfer_proposal_detail(identity.user_id, proposal_id)
        )
    ...
    def capital_transfer_detail(
        ...
        return cast(
            dict[str, Any],
            queries().capital_transfer_detail(identity.user_id, capital_transfer_id),
        )
