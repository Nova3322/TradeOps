...
class _ExecutionRoutes:
    def __init__(self, context: ApiRouteContext) -> None:
        dependencies = context.execution
        common = dependencies.common
        self.app = context.app
        self.current_perptape_candidate = dependencies.current_perptape_candidate
        self.current_perptape_candidates = dependencies.current_perptape_candidates
        self.identity_dependency = common.identity
        self.notify_campaign = dependencies.notify_campaign
    ...
    def register_campaign_control(self) -> None:
        ...
        def campaigns(
            ...
            self.require_capability(identity, "operations.view")
            return {
                "data": self.queries().list_campaigns(identity.user_id),
                "as_of": _now().isoformat(),
            }

        @self.app.get("/api/campaign-exceptions")
        def campaign_exceptions(
            ...
            now = _now()
            return {
                "data": self.queries().list_exceptions(identity.user_id, now=now),
                "as_of": now.isoformat(),
            }
        ...
        @self.app.get("/api/campaigns/{campaign_id}/add-candidates")
        def campaign_add_candidates(
            ...
            proposal = self.queries().proposal_detail(
                identity.user_id, UUID(str(detail["proposal_id"]))
            )
            source_candidate_id = proposal.get("source_candidate_id")
            now = _now()
            candidates = [
                item
                for item in self.current_perptape_candidates(user_id=identity.user_id, now=now)
                if item.venue == detail["venue"]
                and item.symbol == symbol
                and item.direction.value == detail["direction"]
                and item.readiness == "READY"
                and source_candidate_id
                not in {item.candidate_id, perptape_legacy_candidate_id(item)}
            ]
            return {
                "source": "PERPTAPE",
                "source_contract_version": self.resolved_settings.perptape_contract_version,
                "as_of": now.isoformat(),
                "data": [item.to_dict() for item in candidates],
            }

        @self.app.post("/api/campaigns/{campaign_id}/auto-add")
        def create_auto_add_intent(
            ...
            created = self.service().create_order_intent(
            ...
            return {
                ...
                "detail": self.queries().campaign_detail(identity.user_id, created.campaign_id),
            }

        @self.app.post("/api/campaigns/{campaign_id}/managed-reductions")
        def create_managed_reduction(
            campaign_id: UUID,
            payload: ManagedReductionRequest,
            identity: SessionIdentity = self.identity_dependency,
        ) -> dict[str, Any]:
            intent_id = self.service().create_reduction_intent(
                campaign_id,
                identity.user_id,
                payload.idempotency_key,
                candidates=(
                    TargetCandidate(payload.target_quantity, payload.urgency, payload.reason),
                ),
        ...
        @self.app.post("/api/campaigns/{campaign_id}/automatic-exit")
        def evaluate_automatic_exit(
            campaign_id: UUID,
            payload: AutomaticExitRequest,
            identity: SessionIdentity = self.identity_dependency,
        ) -> dict[str, Any]:
            reason, intent_id = self.service().create_automatic_exit_intent(
                campaign_id,
                identity.user_id,
                payload.idempotency_key,
                limit_price=payload.limit_price,
        ...
        @self.app.post("/api/campaigns/{campaign_id}/auto-add/disable")
        def disable_campaign_auto_add(
            campaign_id: UUID,
            payload: RiskTightenRequest,
            identity: SessionIdentity = self.identity_dependency,
        ) -> dict[str, Any]:
            allowed_adds = self.service().disable_campaign_auto_add(
                campaign_id,
                identity.user_id,
                payload.idempotency_key,
        ...
        @self.app.post("/api/operations/auto-add/disable")
        def disable_global_auto_add(
            payload: RiskTightenRequest,
            identity: SessionIdentity = self.identity_dependency,
        ) -> dict[str, str]:
            self.service().disable_global_auto_add(
                identity.user_id,
                payload.idempotency_key,
                reason=payload.reason,
                now=_now(),
        ...
        def pause_new_risk(
            ...
            state = self.service().pause_new_risk(
                identity.user_id,
                payload.idempotency_key,
                reason=payload.reason,
                now=_now(),
    ...
    def register_intent(self) -> None:
        @self.app.post("/api/authorizations/{authorization_id}/intents")
        ...
        @self.app.post("/api/facts/positions")
        def record_position(
            payload: PositionFactRequest,
            identity: SessionIdentity = self.identity_dependency,
        ) -> dict[str, str]:
            position_id = self.service().record_position(
                payload.account_id,
                payload.venue,
                payload.instrument_id,
                payload.quantity,
        ...
        def record_account_equity(
            ...
            fact_id = self.service().record_account_equity(
                payload.account_id,
                payload.venue,
                payload.equity,
        ...
        def acquire_sender(
            ...
            now = _now()
            token = self.service().acquire_sender(
    ...
    def register_fact_campaign(self) -> None:
        ...
        def mark_intent_unknown(
            ...
            campaign_id = self.queries().campaign_id_for_intent(identity.user_id, intent_id)
            self.service().mark_intent_unknown(
                intent_id,
                identity.user_id,
                payload.reason,
        ...
        @self.app.post("/api/intents/{intent_id}/release")
        def release_intent(
            intent_id: UUID,
        ...
        @self.app.post("/api/campaigns/{campaign_id}/protection")
        def record_protection(
            campaign_id: UUID,
            payload: ProtectionFactRequest,
            identity: SessionIdentity = self.identity_dependency,
        ) -> dict[str, Any]:
            self.queries().campaign_detail(identity.user_id, campaign_id)
            protection_id = self.service().record_protection(
                payload.position_id,
                payload.venue_order_id,
                payload.quantity,
                payload.trigger_price,
        ...
        @self.app.post("/api/campaigns/{campaign_id}/funding")
        def record_funding(
            campaign_id: UUID,
            payload: FundingFactRequest,
            identity: SessionIdentity = self.identity_dependency,
        ) -> dict[str, Any]:
            payment_id = self.service().record_funding(
                campaign_id,
                payload.venue,
        ...
        @self.app.post("/api/campaigns/{campaign_id}/target")
        def update_campaign_target(
            campaign_id: UUID,
            payload: CampaignTargetRequest,
            identity: SessionIdentity = self.identity_dependency,
        ) -> dict[str, Any]:
            decision = self.service().update_campaign_target(
                campaign_id,
                identity.user_id,
                tuple(
        ...
        @self.app.post("/api/campaigns/{campaign_id}/reduction-intents")
        ...
        @self.app.post("/api/campaigns/{campaign_id}/reconcile")
        def reconcile_campaign(
            campaign_id: UUID,
            payload: ReconciliationRequest,
            identity: SessionIdentity = self.identity_dependency,
        ) -> dict[str, Any]:
            reconciliation_id = self.service().reconcile_campaign(
        ...
        @self.app.post("/api/campaigns/{campaign_id}/freqtrade/emergency-exit/recover")
        def recover_freqtrade_emergency_exit(
            campaign_id: UUID,
            payload: ReconciliationReasonRequest,
            identity: SessionIdentity = self.identity_dependency,
        ) -> dict[str, Any]:
            intent_id = self.service().recover_freqtrade_emergency_exit(
                campaign_id,
                identity.user_id,
                payload.reason,
                now=_now(),
        ...
        @self.app.post("/api/reconciliations/{reconciliation_id}/manual")
        def require_manual_reconciliation(
            reconciliation_id: UUID,
            payload: ReconciliationReasonRequest,
            identity: SessionIdentity = self.identity_dependency,
        ) -> dict[str, str]:
            result = self.service().require_manual_reconciliation(
                reconciliation_id, identity.user_id, payload.reason, now=_now()
            )
            return {"reconciliation_id": str(result), "status": "MANUAL_REQUIRED"}

        @self.app.post("/api/reconciliations/{reconciliation_id}/resolve")
        def resolve_reconciliation(
            reconciliation_id: UUID,
            payload: ReconciliationReasonRequest,
            identity: SessionIdentity = self.identity_dependency,
        ) -> dict[str, str]:
            result = self.service().resolve_reconciliation(
                reconciliation_id, identity.user_id, payload.reason, now=_now()
            )
            return {"reconciliation_id": str(result), "status": "RESOLVED"}

        @self.app.post("/api/campaigns/{campaign_id}/pnl")
        def refresh_campaign_pnl(
            campaign_id: UUID,
            identity: SessionIdentity = self.identity_dependency,
        ) -> dict[str, Any]:
            pnl = self.service().refresh_campaign_pnl(campaign_id, identity.user_id, now=_now())
            return {
                "pnl": {
        ...
        @self.app.post("/api/campaigns/{campaign_id}/close")
        def close_campaign(
            campaign_id: UUID,
            identity: SessionIdentity = self.identity_dependency,
        ) -> dict[str, Any]:
            self.service().close_campaign(campaign_id, identity.user_id, now=_now())
            self.notify_campaign(
                identity.user_id,
                campaign_id,
                "CAMPAIGN_CLOSED",
    ...
    def register_runtime(self) -> None:
        ...
        def actual_results(
            ...
            return {
                "data": self.queries().actual_results(
                    identity.user_id,
                    environment,
                    source=source,
        ...
        def audit_timeline(
            ...
            return {
                ...
                "data": self.queries().audit_timeline(identity.user_id, environment, limit=limit),
                "as_of": _now().isoformat(),
            }
        ...
        def runtime_status(
            ...
            signal_source = self.service().signal_source_status(identity.user_id)["source"]
            ...
            snapshot.update(
                {
                    ...
                    "runtime_environment": self.resolved_settings.environment,
                    "process_model": (
                        "FastAPI plus independent read-only sync worker and PostgreSQL"
                        if self.resolved_settings.runtime_sync_enabled
                    ...
                    "external_boundaries": {
                        ...
                        "perptape": {
                            ...
                            "contract_version": self.resolved_settings.perptape_contract_version,
                        ...
                        "notilt": {
                            ...
                            "configured_chains": sorted(self.resolved_settings.notilt_vaults),
                            "receipt_verification": True,
                            "min_confirmations": self.resolved_settings.notilt_min_confirmations,
                        ...
                        "capital_transfer": {
                            ...
                            "real_configured": (
                                self.resolved_settings.notilt_enabled
                                and bool(self.resolved_settings.notilt_vaults)
                            ),
                        },
                        "telegram": {
                            "mode": (
        ...
        @self.app.get("/api/execution/freqtrade/status")
...
def register_execution_routes(context: ApiRouteContext) -> None:
    """Register execution routes from bounded lifecycle groups."""
