...
def register_proposals_routes(context: ApiRouteContext) -> None:
    """Register proposals routes against one application dependency context."""
    ...
    opportunity_snapshot = context.require("opportunity_snapshot")
    ...
    def opportunities(
        ...
        return cast(dict[str, Any], opportunity_snapshot(user_id=identity.user_id, now=_now()))
    ...
    def proposal_detail(
        ...
        require_capability(identity, "proposal.view")
        return cast(
            dict[str, Any], queries().proposal_detail(identity.user_id, proposal_id, now=_now())
        )
