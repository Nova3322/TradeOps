...
from trading_control_plane import models
from trading_control_plane.query_component import QueryComponent


class CampaignQueryReader(Protocol):
...
def list_exceptions(
    scope: QueryComponent,
    campaigns: CampaignQueryReader,
    user_id: UUID,
    *,
    now: datetime,
    ...
    for campaign in campaigns.list_campaigns(user_id):
        if campaign["status"] == "CLOSED":
            continue
        detail = campaigns.campaign_detail(user_id, UUID(str(campaign["campaign_id"])))
            ...
            if intent["status"] == "DISPATCHING":
                exceptions.append(
                    exception_projection(
                        ...
                        occurred_at=str(intent["dispatch"]["started_at"] or intent["updated_at"]),
                    )
                )
        ...
        if position is None or position["fact_status"] == "UNKNOWN":
            ...
            if scope_rules.fact_is_stale(position_observed_at, now, max_fact_age):
                exceptions.append(
                    exception_projection(
                        campaign_id,
                        "POSITION_STALE",
        ...
        if (
            ...
            if protection is None or protection["status"] == "UNKNOWN":
                ...
                if scope_rules.fact_is_stale(protection_observed_at, now, max_fact_age):
                    exceptions.append(
                        exception_projection(
                            campaign_id,
                            "PROTECTION_STALE",
        ...
        if reconciliation is not None and reconciliation["status"] != "MATCH":
            ...
            if (
                detail["intents"]
                and datetime.fromisoformat(str(detail["intents"][-1]["updated_at"])) > completed_at
            ):
                newer_facts.append(
                    (
                        "ORDER_INTENT_NEWER",
