async function renderManualProposal() {
  ...
  const environmentCopy = fmtCompactEnvironment(environment);
  const environmentMeaning = fmtEnvironment(environment, true);
  main.innerHTML = `<section class="page"><header class="page-head"><div><p class="eyebrow">人工创建交易提案</p><h1>创建人工提案</h1><p class="lede">先说明交易方向、计划规模和最多可以承受的损失。分批入场和自动加仓是高级选项；提交后只会保存当前判断，仍需独立审核和系统风险检查。</p></div><span class="status-pill ${environment === 'TESTNET' ? 'status-APPROVED' : ''}" title="${escapeHtml(environmentMeaning)}" aria-label="${escapeHtml(environmentMeaning)}">${escapeHtml(environmentCopy)}</span></header>
    <div class="compose-layout"><form id="manual-form" class="form-panel proposal-compose" data-environment="${escapeHtml(environment)}">
      <section class="form-section"><div class="section-title"><span>1</span><div><h2>交易意图</h2><p>选标的、定方向，说清从哪个价格开始执行。</p></div></div><div class="field-grid proposal-intent-grid">
        <label class="proposal-account-field">账户<span class="field-help">仅显示当前团队与交易所中获权的精确账户</span><select name="account_id" required></select></label>
        <label class="proposal-venue-field">交易所<span class="field-help">先选交易所，再输入币对</span><select name="venue" required><option value="BINANCE">币安</option><option value="HYPERLIQUID">Hyperliquid</option><option value="OKX">OKX</option><option value="BYBIT">Bybit</option></select></label>
        <label class="proposal-symbol-field">币对<span class="field-help">输入完整币对</span><input name="instrument_symbol" list="manual-instrument-options" autocomplete="off" spellcheck="false" placeholder="例如 BTCUSDT" required><input name="instrument_id" type="hidden"><datalist id="manual-instrument-options"></datalist><span class="instrument-match" data-instrument-match data-state="idle" role="status" aria-live="polite"></span></label>
        <label class="proposal-direction-field">方向<span class="field-help">做多或做空</span><select name="direction"><option value="LONG">做多</option><option value="SHORT">做空</option></select></label>
        <label class="proposal-trigger-field">触发价格<span class="field-help">计划开始执行的位置</span><input name="trigger_price" type="number" step="any" min="0" required></label>
      </div></section>
