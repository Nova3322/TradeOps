async function renderManualProposal() {
  ...
  main.innerHTML = `<section class="page"><header class="page-head"><div><p class="eyebrow">人工创建交易提案</p><h1>创建人工提案</h1><p class="lede">先说明交易方向、计划规模和最多可以承受的损失。分批入场和自动加仓是高级选项；提交后只会保存当前判断，仍需独立审核和系统风险检查。</p></div><span class="status-pill ${environment === 'SHADOW' ? 'status-APPROVED' : ''}">${escapeHtml(environmentCopy)}</span></header>
    <div class="compose-layout"><form id="manual-form" class="form-panel proposal-compose" data-environment="${escapeHtml(environment)}">
      ...
      <div class="form-error" role="alert"></div><div class="form-actions"><span class="submit-disclosure">提交后保存当前参数并进入审核队列，${environment === 'SHADOW' ? '全程只使用虚拟资金。' : '不会直接下单。'}</span><button class="primary">创建并提交审核</button></div>
    </form>
...
function reviewEnvironmentSelection(search = location.search) {
  ...
  return ['LIVE','SHADOW'].includes(requested) ? requested : 'ALL';
}
...
async function renderProposalList(status, title, historyMode = false) {
  ...
  const reviewCounts = {
    ...
    SHADOW: reviewableItems.filter(item => item.environment === 'SHADOW').length,
  };
  ...
  const createActions = !status && canPropose ? `<div class="toolbar"><a class="secondary" href="${environment === 'SHADOW' ? '/trading-mode' : '/opportunities'}" data-link>${environment === 'SHADOW' ? '返回交易模式' : '查看机会'}</a><a class="primary" href="/proposals/new${environmentQuery}" data-link>新建人工提案</a></div>` : '';
  const reviewEnvironmentSwitcher = status ? `<section class="review-environment-switcher" aria-label="审核环境范围"><div class="review-environment-copy"><b>审核范围</b><span>生产与影子提案保持独立标记；切换筛选不会改变团队交易模式。</span></div><nav class="review-environment-tabs" aria-label="选择审核环境"><a class="${reviewEnvironment === 'ALL' ? 'active' : ''}" href="/reviews" data-link ${reviewEnvironment === 'ALL' ? 'aria-current="page"' : ''}>全部 <b>${reviewCounts.ALL}</b></a><a class="${reviewEnvironment === 'LIVE' ? 'active' : ''}" href="/reviews?environment=LIVE" data-link ${reviewEnvironment === 'LIVE' ? 'aria-current="page"' : ''}>生产 <b>${reviewCounts.LIVE}</b></a><a class="${reviewEnvironment === 'SHADOW' ? 'active' : ''}" href="/reviews?environment=SHADOW" data-link ${reviewEnvironment === 'SHADOW' ? 'aria-current="page"' : ''}>影子 <b>${reviewCounts.SHADOW}</b></a></nav></section>` : '';
  ...
  main.innerHTML = `<section class="page"><header class="page-head"><div><p class="eyebrow">提案审核</p><h1>${escapeHtml(title)}</h1><p class="lede">${status ? '统一汇总当前团队内需要你独立判断、尚未到期且尚未投票的提案。生产与影子环境始终分开标记；批准不等于下单。' : historyMode ? proposalHistoryCopy : proposalScopeCopy}</p></div>${createActions}</header>
...
async function renderProposalDetail(id) {
  ...
  if (!['LIVE','SHADOW'].includes(item.environment)) {
    main.innerHTML = '<section class="page"><section class="empty-state"><div><h1>该提案不属于当前控制台</h1><p>这里只展示生产或严格隔离的影子提案。</p><a class="primary" href="/proposals" data-link>返回提案列表</a></div></section></section>';
    return;
  }
  const shadowProposal = item.environment === 'SHADOW';
  const detailParams = new URLSearchParams(location.search);
  const detailOrigin = detailParams.get('from');
  const reviewEnvironment = ['LIVE', 'SHADOW'].includes(detailParams.get('environment'))
  ...
  const returnDestination = detailOrigin === 'reviews' && hasCapability('proposal.review')
    ...
    : detailOrigin === 'history'
      ? {href:'/proposals?history=1', label:'历史提案'}
      : {href:'/proposals', label:shadowProposal ? '影子提案' : '当前提案'};
