...
async function renderProposalList(status, title, historyMode = false) {
  ...
  main.innerHTML = `<section class="page"><header class="page-head"><div><p class="eyebrow">提案审核</p><h1>${escapeHtml(title)}</h1><p class="lede">${status ? '统一汇总当前团队内需要你独立判断、尚未到期且尚未投票的提案。生产与测试环境始终分开标记；批准不等于下单。' : historyMode ? proposalHistoryCopy : proposalScopeCopy}</p></div>${createActions}</header>
...
async function renderProposalDetail(id) {
  ...
  if (!['LIVE','TESTNET'].includes(item.environment)) {
    main.innerHTML = '<section class="page"><section class="empty-state"><div><h1>该提案不属于当前控制台</h1><p>这里只展示生产或严格隔离的测试提案。</p><a class="primary" href="/proposals" data-link>返回提案列表</a></div></section></section>';
    return;
  }
  ...
  const returnDestination = detailOrigin === 'reviews' && hasCapability('proposal.review')
    ...
    : detailOrigin === 'history'
      ...
      : {href:'/proposals', label:testnetProposal ? '测试提案' : '当前提案'};
