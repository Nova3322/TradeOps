...
function renderRiskPolicyConfiguration(policy, allowed) {
  ...
  const positionValue = (name, fallback = '') => policy?.position_policy?.[name] ?? fallback;
...
async function bindRiskControlActions() {
  document.querySelector('#risk-policy-form')?.addEventListener('submit', async event => {
    ...
    await withPending(button, form.dataset.riskPolicyWorkflow === 'DIRECT' ? '保存中…' : '冻结提案中…', async () => { try {
      const requestedPolicy = {version:data.version, max_total_risk:data.max_total_risk, max_account_risk:data.max_account_risk, max_single_loss:data.max_single_loss, max_consecutive_losses:Number(data.max_consecutive_losses), loss_cooldown_seconds:Number(data.loss_cooldown_seconds), max_fact_age_seconds:Number(data.max_fact_age_seconds), maximum_position_notional:data.maximum_position_notional, auto_add_spacing_bps:Number(data.auto_add_spacing_bps), auto_add_bollinger_midline_periods:Number(data.auto_add_bollinger_midline_periods), low_maximum_adds:Number(data.low_maximum_adds), medium_maximum_adds:Number(data.medium_maximum_adds), high_maximum_adds:Number(data.high_maximum_adds), low_maximum_loss_fraction:data.low_maximum_loss_fraction, medium_maximum_loss_fraction:data.medium_maximum_loss_fraction, high_maximum_loss_fraction:data.high_maximum_loss_fraction};
