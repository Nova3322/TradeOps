...
async function renderCapitalCenter() {
  ...
  const proposalRows = item.proposals.map(proposal => {
    ...
    return `<tr><td>${shortId(proposal.transfer_proposal_id)}<br><span class="subtle">v${proposal.version}</span></td><td>${escapeHtml(proposal.direction)}</td><td>${escapeHtml(proposal.source_id)} → ${escapeHtml(proposal.destination_id)}</td><td>${fmtNumber(proposal.amount)} ${escapeHtml(proposal.asset)}</td><td><b>${escapeHtml(proposal.status)}</b></td><td><div class="toolbar">${actions.join('')}</div></td></tr>`;
  }).join('');
