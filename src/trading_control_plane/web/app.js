...
const environmentLabels = {LIVE:'实盘',SHADOW:'模拟',TESTNET:'测试网',production:'生产站',test:'测试站',development:'开发环境'};
...
const fmtEnvironment = (value, withCode = false) => {
  const label = environmentLabels[value] || environmentLabels[String(value || '').toLowerCase()] || value || '未知环境';
  return withCode && value ? `${label}（${String(value).toUpperCase()}）` : label;
};
...
const capabilityLabel = (capability) => ({'opportunity.view':'查看机会','proposal.view':'查看提案','operations.view':'交易运维','system.view':'查看系统状态','venue.view':'查看交易所事实','results.view':'查看结果与审计','capital.view':'资金管理','proposal.create':'发起提案','proposal.review':'独立审核','access.manage':'成员权限管理'}[capability] || capability);
const accessRoleCatalog = [
  {role:'OBSERVER', label:'只读观察', copy:'查看机会、提案、交易任务、系统状态、交易所事实和结果；不能执行动作。'},
  ...
  {role:'TREASURY_ADMIN', label:'资金管理', copy:'查看与管理资金事实、划转和资金对账；与交易运维职责分离。'},
...
function errorView(error, retry = true) {
  ...
  return `<section class="error-state"><div><h2>${escapeHtml(title)}</h2><p>${escapeHtml(friendlyApiError(error))}</p>${retry ? '<button class="secondary" data-retry>重新检查</button>' : ''}<details class="technical-details"><summary>技术详情</summary><p class="error-code">错误代码：${escapeHtml(error?.code || 'UNKNOWN')}</p></details></div></section>`;
}
...
async function bootstrap() {
  ...
  environmentBadge.textContent = fmtEnvironment(authStatus.environment || 'development', true);
  try {
...
function renderLogin() {
  main.innerHTML = `<section class="login-page"><div class="login-card">
    <span class="mock-ribbon">${authStatus.mock_identity_available ? '非生产身份模拟' : '需要统一身份登录'}</span>
    <p class="eyebrow" style="margin-top:18px">内部访问</p><h1>进入交易控制台</h1>
    <p class="lede">系统不开放外部注册。正式环境使用统一身份登录和通行密钥（Passkey）；本地模拟登录只用于验证已经存在的内部用户。</p>
    ${sessionNotice ? `<div class="callout" role="status">${escapeHtml(sessionNotice)}</div>` : ''}
    ${authStatus.mock_identity_available ? `<form id="login-form"><label>内部用户名<input name="username" autocomplete="username" required placeholder="reviewer-1"></label><button class="primary">使用模拟身份进入</button><div class="form-error" role="alert"></div></form>` : '<div class="callout">统一身份服务尚未接入。系统不会自动降级为本地密码登录。</div>'}
  </div></section>`;
...
async function renderOpportunities() {
  ...
  main.innerHTML = `<section class="page"><header class="page-head"><div><p class="eyebrow">PERPTAPE · ${escapeHtml(result?.source_contract_version || '连接不可用')}</p><h1>机会</h1><p class="lede">先查看 Perptape 候选；没有合适信号时，也可以点击“人工提案”自行录入。无论哪种方式，都只会创建提案，并且必须经过独立审核。</p></div><div class="toolbar">${canPropose ? '<a class="primary" href="/proposals/new" data-link>＋ 人工提案</a>' : ''}<button class="secondary" data-refresh>刷新机会</button></div></header>
    ${sourceError ? `<article class="source-status tone-attention"><div><p class="eyebrow">Perptape 数据源</p><h2>外部机会当前不可用</h2><p>${escapeHtml(friendlyApiError(sourceError))} 系统不会用过期或模拟候选冒充实时机会。</p></div>${canPropose ? '<a class="secondary" href="/proposals/new" data-link>创建人工提案</a>' : ''}</article>` : ''}
...
async function loadCampaignDetails() {
  ...
  const visible = authStatus?.environment === 'production'
    ? result.data.filter(item => item.environment === 'LIVE')
    : result.data;
...
function renderRiskControlPanel(control) {
  ...
  const requestCards = control.requests.length ? control.requests.map(item => {
    ...
    const executeUi = item.status === 'APPROVED' && isAdmin
      ...
      : '';
    return `<article class="card"><div class="card-head"><div><p class="eyebrow">恢复申请 · 版本 ${item.version}</p><h2>${riskControlStatusLabel(item.status)}</h2></div><span class="tag">${shortId(item.request_id)}</span></div><dl class="definition-grid">${definition('申请人', shortId(item.requester_id))}${definition('恢复自动加仓', item.restore_auto_add ? '是' : '否')}${definition('最早执行', fmtDate(item.execute_after))}${definition('到期', fmtDate(item.expires_at))}${definition('风险政策版本', `${escapeHtml(item.source_policy_version)} · 修订 ${item.source_policy_revision}`)}${definition('控制开关版本', `${escapeHtml(fmtStatus(item.source_auto_add_status))} · 版本 ${item.source_auto_add_version}`)}</dl><p>${escapeHtml(item.reason)}</p><h3>审核记录</h3>${item.reviews.length ? item.reviews.map(review => `<div class="callout"><b>${escapeHtml(review.decision === 'APPROVE' ? '批准' : '拒绝')}</b> · ${escapeHtml(review.reason)}<br><span class="subtle">${shortId(review.reviewer_id)} · ${fmtDate(review.created_at)}</span></div>`).join('') : '<p class="subtle">尚无审核票。</p>'}<div class="review-action-panel">${reviewUi}${executeUi}</div></article>`;
  }).join('') : '<section class="empty-state"><div><h2>尚无恢复申请</h2><p>收紧控制不会自动恢复；需要创建冻结申请并完成双人独立审核。</p></div></section>';
  return `<section class="risk-control-overview"><div class="stats"><div class="stat"><small>风险政策</small><b>${riskControlStatusLabel(policy.system_state)}</b></div><div class="stat"><small>风险政策版本</small><b>${escapeHtml(policy.version)} · 修订 ${policy.revision}</b></div><div class="stat"><small>自动加仓</small><b>${riskControlStatusLabel(gate.status)} · 版本 ${gate.version}</b></div><div class="stat"><small>恢复条件</small><b>${restoreGateLabel}</b></div></div><div class="detail-layout"><article class="card"><h2>当前控制状态</h2><dl class="definition-grid">${definition('政策原因', policy.reason)}${definition('政策更新人', shortId(policy.updated_by))}${definition('政策更新时间', fmtDate(policy.updated_at))}${definition('控制原因', gate.reason)}${definition('控制操作人', shortId(gate.operator_id))}${definition('控制更新时间', fmtDate(gate.updated_at))}</dl><p class="safety-note">“仅允许减仓”仍允许减仓与退出；暂停新增风险后，当时所有未过期的新风险授权都会永久失效。</p></article><article class="card"><h2>实时恢复条件</h2>${conditions.blockers.length ? `<ul class="exception-list">${conditions.blockers.map(item => `<li><code>${escapeHtml(item)}</code></li>`).join('')}</ul>` : `<p class="success-note">${conditions.live_scope_required ? '生产实盘恢复条件当前无阻塞' : '当前本地读取未发现阻塞'}；执行时仍会重新验证。</p>`}<h3>受控范围来源</h3>${conditions.required_scopes.length ? conditions.required_scopes.map(scope => `<div class="callout"><b>${escapeHtml(fmtEnvironment(scope.environment, true))}</b> · ${escapeHtml(scope.account_id)} · ${escapeHtml(scope.venue)}</div>`).join('') : `<p class="danger-note">${conditions.live_scope_required ? '尚未配置生产实盘受控范围，因此恢复执行保持关闭。' : '当前为本地模式且未配置受控范围；生产环境会拒绝执行恢复。'}</p>`}</article></div>${requestForm}<div class="section-head"><div><p class="eyebrow">双人复核流程</p><h2>恢复申请与独立审核</h2></div></div><div class="stack">${requestCards}</div></section>`;
}
...
async function renderCampaignList() {
  const result = await api('/api/campaigns');
  const productionOnly = authStatus?.environment === 'production';
  const items = productionOnly ? result.data.filter(item => item.environment === 'LIVE') : result.data;
  ...
  main.innerHTML = `<section class="page"><header class="page-head"><div><p class="eyebrow">交易任务</p><h1>交易任务</h1><p class="lede">每个交易任务覆盖一笔交易从授权、风险占用和下单意图，到成交、保护、减仓、对账与最终结果的完整生命周期。</p></div><div class="toolbar"><a class="secondary" href="/proposals" data-link>查看提案</a></div></header>
    <div class="stats"><div class="stat"><small>全部交易任务</small><b>${items.length}</b></div><div class="stat"><small>建仓中 / 持仓中</small><b>${items.filter(i => ['OPEN','OPENING'].includes(i.status)).length}</b></div><div class="stat"><small>结果未知</small><b>${items.filter(i => i.status === 'UNKNOWN').length}</b></div><div class="stat"><small>数据环境</small><b style="font-size:14px">${escapeHtml(environments.map(value => fmtEnvironment(value, true)).join(' / ') || '尚无数据')}</b></div></div>
    ${items.length ? `<div class="table-wrap"><table><thead><tr><th>交易任务</th><th>账户范围</th><th>方向 / 目标</th><th>状态</th><th>盈亏</th><th>更新时间</th></tr></thead><tbody>${items.map(item => `<tr data-href="/campaigns/${item.campaign_id}"><td><b>${shortId(item.campaign_id)}</b><br><span class="subtle">提案 ${shortId(item.proposal_id)}</span></td><td>${escapeHtml(item.account_id)}<br><span class="subtle">${escapeHtml(fmtEnvironment(item.environment, true))} · ${escapeHtml(item.venue)}</span></td><td class="${item.direction === 'LONG' ? 'direction-long' : 'direction-short'}">${escapeHtml(fmtDirection(item.direction))} · ${fmtNumber(item.current_target_quantity)}</td><td><b class="status-${escapeHtml(item.status)}">${escapeHtml(fmtStatus(item.status))}</b></td><td>${fmtNumber(item.final_pnl)}</td><td>${fmtDate(item.updated_at)}</td></tr>`).join('')}</tbody></table></div>` : `<section class="empty-state"><div><h2>当前没有${productionOnly ? '实盘' : ''}交易任务</h2><p>${productionOnly ? '生产站只展示实盘交易任务，不使用模拟或测试网数据填充。' : '提案通过审核和风险检查后，交易运维人员才能发起开仓。'}</p></div></section>`}</section>`;
  bindLinkedRows();
}
...
async function renderSystemStatus() {
  ...
  const campaigns = campaignsResponse.data.filter(item => item.status !== 'CLOSED' && (authStatus?.environment !== 'production' || item.environment === 'LIVE'));
  ...
  const cards = [
    systemHealthCard({title:'核心服务', status:health.ready ? '服务可用' : '服务不可用', tone:health.ready ? 'success' : 'danger', copy:health.ready ? '数据库和交易事务服务已通过就绪检查。' : '就绪检查失败；不能把缺失响应当成正常。', meta:'PostgreSQL · 数据缺失即阻断'}),
    systemHealthCard({title:'开仓与加仓', status:controlAvailable ? (addOpen ? '允许新增风险' : entryOpen ? '自动加仓已关闭' : riskControlStatusLabel(policy.system_state)) : '风险政策未配置', tone:addOpen ? 'success' : controlAvailable ? 'attention' : 'danger', copy:controlAvailable ? `风险政策：${riskControlStatusLabel(policy.system_state)}；自动加仓：${riskControlStatusLabel(gate.status)}。` : '缺少当前风险政策或自动加仓控制，系统会阻止新增风险。', meta:`风险政策版本 ${policy.version} · 控制版本 ${gate.version}`}),
  ...
  const venueRows = venueConnections.map(item => `<tr><td><b>${escapeHtml(item.venue)}</b></td><td>${item.enabled && item.configured ? '<span class="status-pill status-APPROVED">连接正常</span>' : `<span class="status-pill">${item.enabled ? '尚未配置' : '已关闭'}</span>`}</td><td>${escapeHtml(fmtEnvironment(item.fact_environment || item.source_environment, true))}</td><td>${item.order_send_available ? '已配置（仍受控制开关限制）' : '尚未配置'}</td><td><a class="text-button" href="/venues?venue=${encodeURIComponent(item.venue)}" data-link>查看账户数据 →</a></td></tr>`).join('');
  const perptapeRow = `<tr><td><b>Perptape</b></td><td><span class="status-pill ${perptapeConnected ? 'status-APPROVED' : ''}">${escapeHtml(perptapeStatus)}</span></td><td>外部市场数据</td><td>只读，不发送订单</td><td><a class="text-button" href="/opportunities" data-link>查看机会 →</a></td></tr>`;
...
async function renderCampaignFacts(mode) {
  ...
  const headers = mode === 'positions' ? '<th>交易任务</th><th>标的</th><th>仓位</th><th>数据状态</th><th>保护</th><th>观测时间</th>' : mode === 'orders' ? '<th>交易任务</th><th>意图</th><th>方向 / 数量</th><th>状态</th><th>交易所订单</th><th>更新时间</th>' : '<th>交易任务</th><th>状态</th><th>风险预留</th><th>目标</th><th>紧迫度</th><th>对账</th>';
  main.innerHTML = `<section class="page"><header class="page-head"><div><p class="eyebrow">数据来源：PostgreSQL</p><h1>${titles[mode]}</h1><p class="lede">这里直接读取当前保存的数据；可以重新计算的汇总不会重复保存。</p></div></header>
...
function shadowFactsForm() {
  return `<form id="shadow-facts-form" class="form-panel compact-form"><h2>记录当前模拟数据</h2><p class="safety-note">这是交易运维人员明确录入的非生产合成数据，不来自真实交易所，也不会被标记为真实账户数据。</p><div class="field-grid"><label>账户<input name="account_id" value="acct-1" required></label><label>交易所<input name="venue" value="BINANCE" required></label><label>合约编号<input name="instrument_id" required></label><label>仓位数量<input name="quantity" type="number" step="any" value="0" required></label><label>平均入场价<input name="average_entry_price" type="number" step="any" value="0" required></label><label>标记价<input name="mark_price" type="number" step="any" required></label><label>权益<input name="equity" type="number" step="any" required></label><label>可用余额<input name="available_balance" type="number" step="any" required></label><label>币种<input name="currency" value="USDT" required></label></div><div class="form-error"></div><div class="form-actions"><button class="primary">记录模拟数据</button></div></form><div style="height:16px"></div>`;
}
...
async function recordStartingFacts(event) {
  ...
  await withPending(button, '记录中…', async () => {
    ...
    try {
      ...
      showToast('当前模拟仓位与权益数据已记录'); await route();
    } catch (error) {
      showApiError(error, form.querySelector('.form-error'));
      if (!error.handled) showToast('模拟数据未完整记录，请先核对当前数据再决定是否继续', 'error');
    }
  });
}
...
function capitalBalanceRows(balances) {
  return balances.map(balance => {
    ...
    return `<tr${missing ? ' class="capital-missing-row"' : ''}><td>${escapeHtml(sourceLabel)}<br><span class="subtle">${escapeHtml(balance.location_id)}</span></td><td>${escapeHtml(fmtEnvironment(balance.environment, true))} · ${escapeHtml(balance.venue)}</td><td>${fmtNumber(balance.confirmed_available)} ${escapeHtml(balance.asset)}</td><td>${balance.usd_equity === null ? '未知' : `${fmtNumber(balance.usd_equity)} USD`}</td><td>${fmtNumber(balance.source_reserved)}</td><td><b>${fmtNumber(balance.effective_available)}</b></td><td>${state}</td><td>${missing ? '—' : fmtDate(balance.observed_at)}</td></tr>`;
  }).join('');
}

function capitalBalanceTable(rows, emptyMessage) {
  return rows
    ? `<div class="table-scroll-hint">左右滑动查看完整资金数据</div><div class="table-wrap is-scrollable"><table><thead><tr><th>位置</th><th>环境 / 交易所</th><th>已确认可用</th><th>USD 净值</th><th>源端预留</th><th>有效可用</th><th>控制 / 充值</th><th>观测</th></tr></thead><tbody>${rows}</tbody></table></div>`
...
async function renderCapitalCenter() {
  ...
  const renderTransferRows = records => records.map(transfer => {
    ...
    if (canTreasury && transfer.transport === 'MOCK' && ['SUBMITTED','IN_FLIGHT'].includes(transfer.status)) actions.push(`<button class="danger" data-cap-observe="${transfer.capital_transfer_id}" data-status="UNKNOWN">标记为结果未知</button>`);
  ...
  const capitalProposalForm = canTreasury ? `<form id="capital-proposal-form" class="form-panel compact-form"><h2>模拟资金提案</h2><p class="safety-note">仅创建测试网或模拟环境的提案；不提供实盘创建入口，也不连接资金库、区块链或交易所。</p><div class="field-grid"><label>环境<select name="environment"><option value="TESTNET">测试网</option><option value="SHADOW">模拟</option></select></label><label>方向<select name="direction"><option value="VAULT_TO_VENUE">资金库转入交易所</option><option value="VENUE_TO_VAULT">交易所转回资金库</option></select></label><label>交易账户<input name="account_id" value="acct-1" required></label><label>交易所<input name="venue" value="BINANCE" required></label><label>资金库编号<input name="vault_id" value="vault-1" required></label><label>资产<input name="asset" value="USDT" required></label><label>网络<input name="network" value="TESTNET" required></label><label>目的端引用<input name="destination_reference" value="approved-test-destination" required></label><label>划转总额<input name="amount" type="number" step="any" min="0" required></label><label>最大费用<input name="max_fee" type="number" step="any" min="0" value="1" required></label><label>最小到账<input name="min_received" type="number" step="any" min="0" required></label><label>有效期（分钟）<input name="expires_in_minutes" type="number" min="5" value="120" required></label></div><label>理由<textarea name="reason" rows="2" required>人工调配资金</textarea></label><button class="primary">创建模拟草稿</button></form>` : '';
  const mockFactForm = canTreasury ? `<form id="capital-fact-form" class="form-panel compact-form"><h2>记录模拟资金数据</h2><p class="safety-note">只写入测试网或模拟环境的观测数据，不连接资金库、区块链或交易所，也不会移动资金。</p><div class="field-grid"><label>位置<select name="location_type"><option value="VAULT">链上资金库</option><option value="VENUE">交易所</option></select></label><label>位置编号<input name="location_id" value="vault-1" required></label><label>交易所<input name="venue" value="BINANCE" required></label><label>资产<input name="asset" value="USDT" required></label><label>权益<input name="equity" type="number" step="any" required></label><label>已确认可用<input name="available_balance" type="number" step="any" required></label><label>可划出<input name="withdrawable_balance" type="number" step="any" required></label><label>网络<input name="network" value="TESTNET"></label><label>控制状态<select name="control_status"><option value="CONTROLLED">受控</option><option value="READ_ONLY">只读</option><option value="UNKNOWN">未知</option></select></label><label>充值状态<select name="deposit_status"><option value="READY">可用</option><option value="PENDING">等待中</option><option value="UNKNOWN">未知</option></select></label></div><button class="secondary">记录模拟数据</button></form>` : '';
  const policyRows = automation.policies.map(policy => `<tr><td>${escapeHtml(fmtEnvironment(policy.environment, true))} · ${escapeHtml(policy.account_id)} / ${escapeHtml(policy.venue)}<br><span class="subtle">${escapeHtml(policy.asset)} · 版本 ${policy.version}</span></td><td>${fmtNumber(policy.operating_low)} / <b>${fmtNumber(policy.operating_target)}</b> / ${fmtNumber(policy.operating_high)}</td><td>${fmtNumber(policy.vault_minimum_reserve)} / ${fmtNumber(policy.minimum_transfer)}–${fmtNumber(policy.maximum_transfer)}</td><td><div class="toolbar"><button class="secondary" data-cap-scope-reconcile="${policy.policy_id}" data-environment="${policy.environment}" data-account="${escapeHtml(policy.account_id)}" data-venue="${escapeHtml(policy.venue)}">空仓对账</button><button class="secondary" data-cap-auto="${policy.policy_id}" data-purpose="AUTO_PROFIT_SWEEP" ${automation.gates.AUTO_PROFIT_SWEEP !== 'ENABLED' ? 'disabled' : ''}>评估利润归集</button><button class="secondary" data-cap-auto="${policy.policy_id}" data-purpose="AUTO_OPERATING_REFILL" ${automation.gates.AUTO_OPERATING_REFILL !== 'ENABLED' ? 'disabled' : ''}>评估运营补充</button></div></td></tr>`).join('');
  const automationPanel = `<section><h2>自动资金候选</h2><p class="safety-note">利润归集与运营补充分别受独立控制开关约束。系统只生成需要双人复核和独立授权的候选提案，不会自动划转资金。存在浮盈、运行中仓位、未决订单、结果未知或对账不一致时都会阻断。</p><div class="stats"><div class="stat"><small>自动利润归集</small><b style="font-size:14px">${escapeHtml(fmtStatus(automation.gates.AUTO_PROFIT_SWEEP || 'MISSING'))}</b></div><div class="stat"><small>自动运营资金补充</small><b style="font-size:14px">${escapeHtml(fmtStatus(automation.gates.AUTO_OPERATING_REFILL || 'MISSING'))}</b></div></div>${canTreasury ? `<form id="capital-policy-form" class="form-panel compact-form"><h3>模拟 / 测试网运营阈值</h3><div class="field-grid"><label>环境<select name="environment"><option value="TESTNET">测试网</option><option value="SHADOW">模拟</option></select></label><label>交易账户<input name="account_id" value="acct-1" required></label><label>交易所<input name="venue" value="BINANCE" required></label><label>资金库编号<input name="vault_id" value="vault-1" required></label><label>资产<input name="asset" value="USDT" required></label><label>网络<input name="network" value="TESTNET" required></label><label>资金库目的端引用<input name="vault_destination_reference" value="approved-test-vault" required></label><label>交易所目的端引用<input name="venue_destination_reference" value="approved-test-venue" required></label><label>运营下沿<input name="operating_low" type="number" step="any" value="400" required></label><label>运营目标<input name="operating_target" type="number" step="any" value="500" required></label><label>运营上沿<input name="operating_high" type="number" step="any" value="600" required></label><label>资金库最低储备<input name="vault_minimum_reserve" type="number" step="any" value="500" required></label><label>最小划转<input name="minimum_transfer" type="number" step="any" value="10" required></label><label>最大划转<input name="maximum_transfer" type="number" step="any" value="200" required></label><label>最大费用<input name="max_fee" type="number" step="any" value="1" required></label></div><button class="secondary">保存非生产策略</button></form>` : ''}${policyRows ? `<div class="table-wrap"><table><thead><tr><th>范围</th><th>下沿 / 目标 / 上沿</th><th>资金库储备 / 划转限额</th><th>动作</th></tr></thead><tbody>${policyRows}</tbody></table></div>` : '<div class="callout">尚无资金自动化策略；两个控制开关默认关闭。</div>'}</section>`;
  ...
  const simulationPanel = authStatus?.environment === 'production' ? '' : `<details class="simulation-panel"><summary><span><b>模拟数据（模拟 / 测试网）</b><small>独立隔离，不计入实盘净值</small></span><span class="tag">${simulatedCount} 条记录</span></summary><div class="simulation-content"><div class="safety-note"><b>非生产区域：</b>以下表单、余额、提案与划转记录不属于实盘，不参与顶部总净值、资金库净值、实盘在途占用或资金构成曲线。</div>${capitalProposalForm}${mockFactForm}${automationPanel}<section><h3>模拟资金数据</h3>${capitalBalanceTable(simulatedBalanceRows, '尚无模拟或测试网资金数据。')}</section><section><h3>模拟资金提案</h3>${proposalTable(simulatedProposalRows, '尚无模拟或测试网资金提案。')}</section><section><h3>模拟资金划转</h3>${transferTable(simulatedTransferRows, '尚无模拟或测试网划转状态。')}</section></div></details>`;
  main.innerHTML = `<section class="page"><header class="page-head"><div><p class="eyebrow">实盘资金 · 数据缺失即阻断</p><h1>资金中心</h1><p class="lede">默认只展示实盘数据：系统合并 Binance、Hyperliquid 与 NoTilt 链上资金库的已确认数据，计算美元净值；任何未知或过期估值都会把净值标记为不完整。Telegram 只能发送通知，不能批准或执行资金动作。</p></div></header><div class="stats"><div class="stat"><small>实盘总净值</small><b>${fmtNumber(netWorth.total)} ${escapeHtml(netWorth.currency)}</b></div><div class="stat"><small>链上资金库净值</small><b>${fmtNumber(netWorth.vault)} ${escapeHtml(netWorth.currency)}</b></div>${venueNetWorth}<div class="stat"><small>净值状态</small><b style="font-size:14px">${escapeHtml(fmtStatus(netWorth.complete ? 'CURRENT' : 'INCOMPLETE'))}</b></div><div class="stat"><small>实盘划转控制</small><b style="font-size:14px">${escapeHtml(fmtStatus(item.real_transfer_gate || 'DISABLED'))}</b></div><div class="stat"><small>实盘在途 / 占用</small><b>${fmtNumber(liveInTransit)}</b></div></div><section class="capital-chart-panel"><div class="chart-head"><div><p class="eyebrow">实盘资金快照</p><h2>资金构成曲线</h2><p class="subtle">按各实盘资金位置最新有效的美元估值累计；这是当前快照，不是历史净值曲线。</p></div><b>${fmtNumber(netWorth.total)} <small>${escapeHtml(netWorth.currency)}</small></b></div>${chartBalances.length ? `<canvas id="capital-chart" height="210" aria-label="实盘当前资金构成累计曲线"></canvas><div class="chart-legend">${chartLegend}</div>` : '<div class="chart-empty">有效的实盘资金估值就绪后，将在这里显示曲线。</div>'}</section>${netWorth.complete ? '' : `<div class="callout"><b>实盘净值不完整：</b>${escapeHtml((netWorth.issues || []).join('；') || '尚无资金数据')}</div>`}<section><h2>实盘资金、美元估值与源端预留</h2><p class="subtle">固定展示 Binance、Hyperliquid 与链上资金库；金额为 0 且状态为“数据缺失”，表示当前没有可确认数据，并不代表已经完成同步。</p>${capitalBalanceTable(liveBalanceRows, '尚无实盘资金数据。')}</section><section><h2>实盘资金提案</h2>${proposalTable(liveProposalRows, '尚无实盘资金提案。')}</section><section><h2>实盘资金划转</h2>${transferTable(liveTransferRows, '尚无实盘划转状态。')}</section>${simulationPanel}</section>`;
...
function resultsEnvironmentNotice(environment) {
  return {
    SHADOW:'模拟环境仅展示合成记录，不代表交易所执行或真实收益。',
    TESTNET:'测试网仅展示非生产环境记录，不代表真实收益。',
    LIVE:'实盘环境只展示系统实际收到并保存的数据，不代表盈利承诺。',
  }[environment];
}

async function renderActualResults() {
  const environment = new URLSearchParams(location.search).get('environment') || (authStatus?.environment === 'production' ? 'LIVE' : 'SHADOW');
  ...
  const capabilityNames = {LIVE_ORDER_SEND:'实盘下单',CAPITAL_TRANSFER:'资金划转',AUTO_ADD:'自动加仓',AUTO_PROFIT_SWEEP:'自动利润归集',AUTO_OPERATING_REFILL:'自动运营资金补充'};
  const gates = Object.entries(runtime.capability_gates).map(([key, gate]) => `<tr><td>${escapeHtml(capabilityNames[key] || key)}</td><td><b>${escapeHtml(fmtStatus(gate.status))}</b></td><td>${escapeHtml(gate.reason)}</td><td>${fmtDate(gate.updated_at)}</td></tr>`).join('');
  main.innerHTML = `<section class="page results-page"><header class="page-head"><div><p class="eyebrow">交易记录 · ${escapeHtml(fmtEnvironment(environment, true))}</p><h1>交易结果</h1><p class="lede">先看盈亏和当前结论，再查看每个交易任务的成交、成本、对账与审计记录。不同环境的数据始终分开显示。</p></div><label>环境<select id="results-environment"><option value="SHADOW" ${environment === 'SHADOW' ? 'selected' : ''}>模拟</option><option value="TESTNET" ${environment === 'TESTNET' ? 'selected' : ''}>测试网</option><option value="LIVE" ${environment === 'LIVE' ? 'selected' : ''}>实盘</option></select></label></header>
    ...
    <div class="stats results-stats"><div class="stat"><small>交易任务</small><b>${results.campaigns.length}</b></div><div class="stat"><small>已关闭</small><b>${closedCount}</b></div><div class="stat"><small>待处理交易任务</small><b class="${affectedCount ? 'danger-text' : ''}">${affectedCount}</b></div><div class="stat"><small>审计事件</small><b>${auditResponse.data.length}</b></div></div>
    ...
    <section><h2>权限与操作记录</h2><p class="subtle">可以打开提案或交易任务继续追查；关联编号和版本用于定位同一条审计链。</p>${audits ? `<div class="table-wrap"><table><thead><tr><th>时间</th><th>操作者</th><th>事件 / 对象</th><th>原因</th><th>关联编号 / 版本</th></tr></thead><tbody>${audits}</tbody></table></div>` : '<div class="callout">当前身份和环境下没有可见审计事件。</div>'}</section>
    <details class="results-technical"><summary><span><b>系统运行边界与技术状态</b><small>仅在排查连接、功能开关或数据版本时查看</small></span><strong>展开技术详情</strong></summary><div class="results-technical-content"><div class="stats"><div class="stat"><small>数据库</small><b style="font-size:14px">${runtime.database_ready ? '可用' : '不可用'}</b></div><div class="stat"><small>运行环境</small><b style="font-size:14px">${escapeHtml(fmtEnvironment(runtime.runtime_environment, true))}</b></div><div class="stat"><small>数据库版本 / 表</small><b style="font-size:14px">${escapeHtml(runtime.schema_revision)} · ${runtime.business_table_count}</b></div><div class="stat"><small>外部资金</small><b style="font-size:14px">仅模拟</b></div></div><div class="table-wrap"><table><thead><tr><th>系统能力</th><th>状态</th><th>原因</th><th>更新时间</th></tr></thead><tbody>${gates}</tbody></table></div><details class="technical-details"><summary>查看外部边界原始记录</summary><pre>${escapeHtml(JSON.stringify(runtime.external_boundaries, null, 2))}</pre></details></div></details>
  </section>`;
...
async function renderVenueFacts() {
  ...
  main.innerHTML = `<section class="page venue-facts-page"><header class="page-head"><div><p class="eyebrow">交易所账户数据 · 只读</p><h1>交易所事实</h1><p class="lede">Binance 与 Hyperliquid 是同一套账户数据能力下的两个来源。这里用于同步、核对和排障；日常交易主要从交易任务、系统状态和异常页面进入。</p></div><button class="secondary" data-refresh>刷新页面数据</button></header>
    ...
    <div class="stats venue-status-stats"><div class="stat"><small>连接状态</small><b class="${connected ? 'direction-long' : 'warning-text'}">${connected ? '连接正常' : status.enabled ? '尚未配置' : '已关闭'}</b><span>${escapeHtml(venueModeLabels[status.mode] || '只读')}</span></div><div class="stat"><small>数据环境</small><b>${escapeHtml(fmtEnvironment(status.fact_environment || status.source_environment, true))}</b><span>${escapeHtml(venueDetail)}</span></div><div class="stat"><small>内部账户</small><b>${escapeHtml(accountId)}</b><span>${escapeHtml(venue)}</span></div><div class="stat"><small>最后同步</small><b>${fmtDate(lastSync)}</b><span>${lastSync ? '账户数据已保存' : '尚无数据'}</span></div></div>
    <article class="card venue-scope-card"><div><h2>账户与同步</h2><p class="subtle">切换账户时只读取当前身份获准查看的范围。点击同步后，系统会从交易所获取数据、保存并立即运行对账。</p></div><form id="venue-account-form" class="inline-form"><input name="venue" type="hidden" value="${venue}"><label>内部账户<input name="account_id" value="${escapeHtml(accountId)}" required></label><button class="secondary">查看账户</button></form>${canSync ? `<form id="venue-sync-form" class="inline-form"><input name="account_id" type="hidden" value="${escapeHtml(accountId)}"><label>${escapeHtml(venue)} 标的<input name="symbol" value="${symbolDefault}" pattern="${symbolPattern}" required></label><button class="primary">同步并对账</button><span class="form-error" role="alert"></span></form>` : `<p class="safety-note">${connected ? '当前身份只有读取权限，不能触发交易所同步。' : '生产读取连接尚未配置或已关闭。页面只展示已经保存的数据，不会用模拟数据填充。'}</p>`}</article>
...
function venueFactSections(facts) {
  ...
  return `<div class="stats"><div class="stat"><small>权益</small><b>${fmtNumber(facts.equity?.equity)} ${escapeHtml(facts.equity?.currency || '')}</b></div><div class="stat"><small>可用余额</small><b>${fmtNumber(facts.equity?.available_balance)}</b></div><div class="stat"><small>权益状态</small><b style="font-size:14px">${escapeHtml(factStatusLabel(facts.equity?.fact_status))}</b></div><div class="stat"><small>最近对账</small><b style="font-size:14px" class="${reconciliation?.status === 'MATCH' ? 'direction-long' : reconciliation ? 'warning-text' : ''}">${escapeHtml(reconciliation ? fmtStatus(reconciliation.status) : '未运行')}</b><span>${fmtDate(reconciliation?.completed_at)}</span></div></div>
    ...
    ${factTable('当前委托', '<th>交易所订单</th><th>标的</th><th>状态</th><th>成交 / 委托</th><th>内部意图</th><th>观测时间</th>', orders)}
...
async function renderAccessManagement() {
  ...
  const cards = members.map(member => {
    ...
    return `<form class="member-access-card ${member.active ? '' : 'is-inactive'}" data-user-access="${member.user_id}"><div class="member-access-head"><div><h2>${escapeHtml(member.username)}</h2><p>${member.identity_bound ? '已绑定生产身份源' : '等待生产身份源绑定'} · 创建于 ${fmtDate(member.created_at)}</p></div><span class="status-pill ${member.active ? 'status-APPROVED' : ''}">${member.active ? '已启用' : '已停用'}</span></div>
      ...
      <div class="scope-grid"><label>账户范围<input name="account_scope" value="${escapeHtml(scope.account)}" placeholder="留空 = 全部账户" ${member.is_current_user ? 'disabled' : ''}></label><label>交易所范围<input name="venue_scope" value="${escapeHtml(scope.venue)}" placeholder="BINANCE / HYPERLIQUID / 留空" ${member.is_current_user ? 'disabled' : ''}></label><label class="active-toggle"><input name="active" type="checkbox" ${member.active ? 'checked' : ''} ${member.is_current_user ? 'disabled' : ''}>允许登录和使用已分配权限</label></div>
...
async function renderCampaignDetail(id) {
  ...
  main.innerHTML = `<section class="page campaign-detail"><header class="page-head"><div><p class="eyebrow">${escapeHtml(fmtEnvironment(item.environment, true))} · ${escapeHtml(item.venue)}</p><h1>${escapeHtml(item.instrument?.symbol || '交易任务')} ${shortId(item.campaign_id)}</h1><p class="lede"><b class="status-${escapeHtml(item.status)}">${escapeHtml(fmtStatus(item.status))}</b> · ${escapeHtml(fmtDirection(item.direction))} · 当前目标 ${fmtNumber(item.current_target_quantity)}</p></div><a class="secondary" href="/campaigns" data-link>返回交易任务</a></header>
    ...
    <div class="campaign-command-layout"><div class="stack"><article class="card"><div class="card-heading"><div><p class="eyebrow">执行记录</p><h2>意图与成交记录</h2></div><span class="status-pill">${item.intents.length} 个意图 · ${item.fills.length} 笔成交</span></div>${item.intents.length ? item.intents.map(intent => intentCard(intent, item.environment)).join('') : '<p class="subtle">尚无订单意图。</p>'}</article><article class="card"><div class="card-heading"><div><p class="eyebrow">仓位数据</p><h2>仓位与保护事实</h2></div><span class="status-pill ${protectionReady ? 'status-APPROVED' : positionCurrent && hasPosition ? 'status-DENY' : ''}">${!positionCurrent ? '仓位待同步' : hasPosition ? (protectionReady ? '保护完整' : '需要保护') : '当前无仓位'}</span></div><dl class="definition-grid spacious">${definition('仓位数量', item.position ? fmtNumber(item.position.quantity) : '未知')}${definition('平均入场', item.position ? fmtNumber(item.position.average_entry_price) : '—')}${definition('标记价', item.position ? fmtNumber(item.position.mark_price) : '—')}${definition('仓位观测', fmtDate(item.position?.observed_at))}${definition('保护状态', item.protection ? fmtStatus(item.protection.status) : '尚无数据')}${definition('保护数量', item.protection ? fmtNumber(item.protection.quantity) : '—')}${definition('保护触发价', item.protection ? fmtNumber(item.protection.trigger_price) : '—')}${definition('保护观测', fmtDate(item.protection?.observed_at))}</dl></article>${canCreatePositionAction ? `<article class="card risk-reduction-card"><div class="card-heading"><div><p class="eyebrow">降低风险</p><h2>减仓与退出随时可用</h2></div><span class="status-pill">只减险</span></div><p class="subtle">无论新增风险是否暂停，都可以把目标降到更小数量或 0；系统只生成只减仓意图。</p>${targetForm(item)}</article>` : ''}${shadowTools}</div>
      <aside class="stack"><article class="card"><div class="card-heading"><div><p class="eyebrow">风险目标</p><h2>风险预留与唯一目标</h2></div><span class="status-pill">版本 ${item.target_version}</span></div>${item.reservations.map(r => `<div class="callout"><b>${escapeHtml(fmtStatus(r.status))}</b> · ${fmtNumber(r.amount)} ${escapeHtml(item.instrument?.collateral_currency || '')}</div>`).join('') || '<p class="subtle">无风险预留。</p>'}<dl class="definition-grid">${definition('目标数量', fmtNumber(item.current_target_quantity))}${definition('紧迫度', item.target_urgency ? fmtStatus(item.target_urgency) : '尚未设置')}${definition('目标原因', item.target_reason || '—')}</dl></article>${managementPanel(item, addCandidates, addCandidateError, canOperate, canAddNow, active, protectionReady, reconciliationMatched)}<article class="card"><div class="card-heading"><div><p class="eyebrow">对账</p><h2>对账结论</h2></div><span class="status-pill ${reconciliationMatched ? 'status-APPROVED' : item.reconciliation ? 'status-DENY' : ''}">${escapeHtml(item.reconciliation ? fmtStatus(item.reconciliation.status) : '未运行')}</span></div>${item.reconciliation ? `<p class="subtle">完成于 ${fmtDate(item.reconciliation.completed_at)}</p>${item.reconciliation.differences.length ? `<ul class="exception-list">${item.reconciliation.differences.map(value => `<li><code>${escapeHtml(value)}</code></li>`).join('')}</ul>` : '<p class="success-note">内部意图、交易所数据、仓位和保护当前一致。</p>'}` : '<p class="subtle">尚未运行对账；任何不确定结果都必须先对账。</p>'}</article><details class="card technical-details"><summary>发送租约与技术边界</summary><div class="toolbox-content"><dl class="definition-grid">${definition('租约所有者', item.sender_lease?.owner_id)}${definition('防并发令牌', item.sender_lease?.fencing_token)}${definition('到期', fmtDate(item.sender_lease?.expires_at))}</dl><p class="safety-note">${item.environment === 'LIVE' ? '实盘执行由服务端控制开关、短期授权、发送租约和交易所适配器共同约束；本页不会用合成回执冒充实盘结果。' : '当前非生产交易任务只记录合成回执，不连接真实交易所。'}</p></div></details></aside></div></section>`;
...
function targetForm(item) { return `<form id="target-form" class="action-panel"><h3>设定唯一减仓目标</h3><label>减仓后剩余数量<input name="target_quantity" type="number" step="any" min="0" max="${escapeHtml(Math.abs(Number(item.position.quantity)))}" required></label><label>处理速度<select name="urgency"><option value="NORMAL">常规</option><option value="URGENT" selected>紧急</option><option value="IMMEDIATE">立即</option></select></label><label>原因<input name="reason" value="人工降低当前风险" required></label><label>执行限价（Hyperliquid 测试网必填）<input name="limit_price" type="number" step="any" min="0"></label><button class="primary">创建只减仓意图</button><button type="button" class="danger" data-auto-exit>评估失效价并退出</button></form>`; }
