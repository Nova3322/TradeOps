...
function translateChineseText(value) {
  return String(value ?? '')
    ...
    .replaceAll('链上资金库 Vault', 'NoTilt Vault')
...
function formatProposalRationale(value) {
  ...
  return `创建提案时，突破榜单中同一精确合约、同一方向在 ${timeframes} 同时突破。${normalizedRiskCopy}来源与参数已冻结，仍需人工审核；不会自动授权或下单。`;
}
...
function renderRiskControlPanel(control) {
  ...
  const conditionTable = rows => `<div class="table-scroll-hint risk-condition-scroll-hint" data-table-hint>左右滑动查看完整安全条件</div><div class="table-wrap"><table><thead><tr><th>条件 / 范围</th><th>状态</th><th>精确原因</th><th>处理角色</th><th>下一步</th></tr></thead><tbody>${rows}</tbody></table></div>`;
  const passedConditions = passingChecks.length
    ? `<details class="risk-passed-conditions"><summary><span><b>${passingChecks.length} 项已通过</b><small>通过项不占用当前待办；恢复或新增风险时仍会重新校验</small></span><strong>按需查看</strong></summary>${conditionTable(passingRows)}</details>`
    : '';
...
const DIRECT_CAPITAL_PATHS = [
  {path:'VAULT_TO_BINANCE', from:'资金方案', to:'币安', badge:'二选一', action:'检查转入币安条件', copy:'选择 NoTilt Vault 或 Safe Spending Limits，再按对应额度规则转入币安。', steps:['选择资金方案','实时额度预检','人控确认','进入币安']},
  {path:'VAULT_TO_HYPERLIQUID', from:'资金方案', to:'Hyperliquid', badge:'二选一', action:'检查转入 Hyperliquid 条件', copy:'选择 NoTilt Vault 或 Safe Spending Limits，先到授权自有地址，再进入 Hyperliquid。', steps:['选择资金方案','额度预检','到达自有地址','合约入金']},
  {path:'BINANCE_TO_VAULT', from:'币安', to:'资金方案', badge:'二选一', action:'检查币安回流条件', copy:'回流到用户选择的 NoTilt Vault 或 Safe Smart Account。', steps:['提现预检','授权地址','目标入金','回执验证']},
  {path:'HYPERLIQUID_TO_VAULT', from:'Hyperliquid', to:'资金方案', badge:'二选一', action:'检查 Hyperliquid 回流条件', copy:'先从合约提回授权自有地址，再进入所选资金方案。', steps:['合约提现','授权地址','目标入金','回执验证']},
];
...
async function renderCapitalCenter() {
  ...
  chartPanel.innerHTML = `<div class="chart-head"><div><p class="eyebrow">资金净值趋势</p><h2>四条固定资金曲线</h2><p class="subtle">Binance、Hyperliquid、Vault 与三方汇总。汇总只使用 ${Number(netWorth.alignment_tolerance_seconds || 60)} 秒内对齐的三方事实；缺失、过期、错位和断档都不会补零或强行连线。</p></div></div><div class="capital-chart-meta"><span>${escapeHtml(chartCoverage)}</span><span>断档按实际采样节奏的 3 倍判定（至少 ${Number(netWorth.history_gap_tolerance_seconds || 300)} 秒）；纵轴至少保留 0.05% 观察范围</span></div><div class="chart-legend" role="group" aria-label="选择显示的资金曲线">${chartLegend}</div>${hasHistory ? `<div class="capital-chart-wrap"><canvas id="capital-chart" height="300" aria-label="Binance、Hyperliquid、Vault 和三方汇总四条 USD 资金趋势"></canvas><div class="capital-chart-tooltip" role="status" hidden></div></div>` : '<div class="chart-empty">尚无可绘制的资金历史；缺失数据不会补零。</div>'}<div class="capital-change-note ${abnormalChanges.length ? 'is-anomaly' : ''}"><b>${abnormalChanges.length ? '异常变化提示' : '最近变化'}</b><span>${escapeHtml(changeCopy)}</span></div>`;
...
function bindCapitalActions(historySeries = []) {
  ...
  document.querySelectorAll('[data-direct-capital-form]').forEach(form => form.addEventListener('submit', async event => {
    ...
    const treasuryProvider = new FormData(currentForm).get('treasury_provider');
    ...
    await withPending(button, '正在安全校验…', async () => {
      try {
        const result = await api('/api/capital/direct-operations', {
          method:'POST',
          body:JSON.stringify({path, treasury_provider:treasuryProvider, amount, final_confirmed:true, idempotency_key:crypto.randomUUID()}),
        });
