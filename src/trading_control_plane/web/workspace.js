function renderLogin() {
  main.innerHTML = `<section class="login-page"><div class="login-card">
    ...
    <p class="eyebrow" style="margin-top:18px">开源交易控制层</p><h1>实盘交易前的控制层</h1>
    <p class="lede">TradeOps 让交易员、交易 Bot 和 AI Agent 的每笔交易先经过确定性规则、必要审批和完整审计，再发送到交易所。</p>
    <p class="callout tone-attention login-exposure-warning"><b>内部系统，请勿暴露到公网</b></p>
...
async function renderHome() {
  if (!hasCapability('operations.view')) {
    if (hasCapability('proposal.review')) {
      ...
      if (!reviewRows.length) reviewRows.push('<article class="home-priority clear no-action"><span class="priority-number">—</span><div><small>暂无待办</small><b>审核队列已清空</b><p>自己的提案、已投票、已过期或已结束记录不会出现在待办中。</p></div></article>');
  ...
  if (!priorityCards.length) priorityCards.push(observerOnly
    ? '<article class="home-priority clear no-action"><span class="priority-number">✓</span><div><small>暂无待办</small><b>继续观察，不必为了操作而操作</b><p>当前身份可以观察机会，但不能创建提案；如有判断请交由提案发起人保存参数。</p></div></article>'
    : `<a class="home-priority clear" href="/opportunities" data-link><span class="priority-number">✓</span><div><small>暂无待办</small><b>继续观察，不必为了操作而操作</b><p>${canPropose ? '机会只是候选；只有形成清楚交易判断时才创建提案。' : '当前身份可以观察机会，但不能创建提案；如有判断请交由提案发起人保存参数。'}</p></div><strong>查看机会 →</strong></a>`);
  const quickStart = observerOnly ? '' : `<article class="card home-quick-start"><p class="eyebrow">${canPropose ? '新的交易判断' : '市场观察'}</p><h2>${canPropose ? '开始新的判断' : '继续观察市场机会'}</h2><p class="subtle">${canPropose ? '先看机会或写交易判断；这两条路径都只会创建提案，并进入独立审核。' : '当前身份只查看候选，不保存交易参数，也不会从这里新增风险。'}</p><div class="stacked-actions home-quick-actions"><a class="secondary" href="/opportunities" data-link>查看 Perptape 机会</a><a class="secondary" href="/webhook-signals" data-link>查看 Webhook 机会</a>${canPropose ? '<a class="text-link" href="/proposals/new" data-link>创建人工提案 →</a>' : ''}</div></article>`;
  main.innerHTML = `<section class="page home-page"><article class="home-status tone-${safety.tone}"><div><p class="eyebrow">${safety.eyebrow}</p><h1>${escapeHtml(safety.title)}</h1><p>${escapeHtml(safety.copy)}</p></div><a class="primary" href="${safety.href}" data-link>${escapeHtml(safety.action)}</a></article>
    <div class="stats home-stats"><div class="stat"><small>运行告警</small><b class="${exceptions.length ? 'danger-text' : ''}">${exceptionCampaigns.size}</b><span>${exceptions.length ? `${exceptions.length} 项问题` : '无告警'}</span></div><div class="stat"><small>独立审核待办</small><b class="${expiringReviews.length ? 'warning-text' : ''}">${actionableReviews.length}</b><span>${actionableReviews.length ? `生产 ${liveReviewCount} · 测试 ${testnetReviewCount}` : canReview ? '创建者不可审核自己的提案' : '当前身份不是审核人'}</span></div><div class="stat"><small>运行中交易任务</small><b>${activeCampaigns.length}</b><span>${activeCampaigns.length ? '保护与对账需持续有效' : '当前没有活动仓位流程'}</span></div><div class="stat"><small>风险边界</small><b class="${riskLimited ? 'warning-text status-copy' : 'status-copy'}">${escapeHtml(riskPolicySummary)}</b><span>真实下单${escapeHtml(liveOrderSendLabel)} · 自动加仓${escapeHtml(riskControl ? riskControlStatusLabel(riskControl.auto_add_gate.status) : '由管理员控制')}</span></div></div>
