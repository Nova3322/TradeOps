...
const API_AGENT_SYSTEM_PROMPT = `你是 TradingOPS API Agent。连接参数由运行环境注入：BASE_URL、TOKEN、WORKSPACE_ID、TEAM_ID、ACCOUNT_ID。
...
async function renderApiAccess(credentialResult = null) {
  ...
  const createPanel = scopes.length ? `<details class="create-member-panel api-token-create" ${clients.length ? '' : 'open'}><summary><span><b>创建 API Token</b><small>固定 Workspace / Team / Account；明文仅显示一次</small></span><strong>展开</strong></summary><form id="create-api-client-form" class="toolbox-content"><div class="field-grid"><label>Agent 名称<input name="name" pattern="[A-Za-z0-9._-]+" placeholder="例如 my-trading-bot" required></label><label>Workspace / Team / Account<select name="scope" required>${apiScopeOptions(scopes)}</select></label><label>Token 有效天数<input name="expires_in_days" type="number" min="1" max="365" value="90" required></label></div><p class="microcopy">权限来自所属 HUMAN 用户的实时角色，范围固定到所选 Workspace、Team、Account 和 Venue。Token 可停用、轮换或永久撤销；资金、管理、风控恢复和需人工确认的动作仍须在网页完成。</p><div class="form-error" role="alert"></div><button class="primary">创建并显示一次 Token</button></form></details>` : '<article class="api-inline-notice tone-attention"><div><p class="eyebrow">暂无可用范围</p><h3>当前没有可创建 Token 的账户范围</h3><p>请联系团队管理员配置有效成员关系、业务角色和交易账户；系统不会创建通配 Agent。</p></div></article>';
  const clientInventory = cards || '<section class="empty-state api-client-empty"><div><h3>尚未创建 API Token</h3><p>选择一个明确范围后创建；权限始终来自你的当前业务角色。</p></div></section>';
