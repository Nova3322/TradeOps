...
function renderScopeSetup() {
  ...
  main.innerHTML = `<section class="page scope-setup-page"><header class="page-head"><div><p class="eyebrow">Workspace → Team → Account</p><h1>${activeTeam ? escapeHtml(activeTeam.name) : '选择团队边界'}</h1><p class="lede">${escapeHtml(setupReason)}</p></div><span class="status-pill ${activeTeam?.trading_enabled ? 'status-APPROVED' : ''}">${activeTeam ? activeTeam.trading_enabled ? '已启用' : '安全配置中' : '尚未选择团队'}</span></header>
    ...
    ${activeTeam && !activeTeam.trading_enabled ? `<article class="home-status tone-attention"><div><p class="eyebrow">Fail closed</p><h2>团队业务能力尚未开放</h2><p>当前可配置成员、信号源、交易账户、通知和风险政策；生产交易链路保持关闭，交易模式由 Team 的服务端状态统一决定。</p></div><div class="toolbar">${hasCapability('access.manage') ? '<a class="secondary" href="/admin/users" data-link>配置团队成员</a>' : ''}${hasCapability('venue.view') ? '<a class="primary" href="/team-settings" data-link>前往模式设置</a>' : '<span class="status-pill">由团队管理员处理</span>'}</div></article>` : ''}
    ${scopeCreationPanels()}
  </section>`;
...
async function renderAccessManagement() {
  ...
  main.innerHTML = `<section class="page access-page"><header class="page-head"><div><p class="eyebrow">${escapeHtml(activeWorkspace?.name || 'Workspace')} · ${escapeHtml(activeTeam?.name || 'Team')}</p><h1>团队成员与权限</h1><p class="lede">岗位、账户范围和交易所范围只在当前团队生效。同一用户加入另一个团队时必须重新分配最小权限。</p></div><span class="status-pill">${members.filter(item => item.active).length} 名启用成员</span></header>
    ...
    <details class="card access-principles" ${window.matchMedia('(min-width: 1101px)').matches ? 'open' : ''}><summary><span><b>权限分离原则</b><small>审核、交易、资金与身份保持独立授权</small></span><strong><span class="when-closed">展开</span><span class="when-open">收起</span></strong></summary><div class="access-principle-grid"><p><b>审核与发起分开</b><span>审核人不能审核自己的提案；提案发起人不会自动获得执行权限。</span></p><p><b>交易与资金分开</b><span>交易运维人员看不到资金中心；系统管理员拥有最高管理权限，但资金动作仍受实时校验、最终确认和安全开关约束。</span></p><p><b>身份与权限分开</b><span>密码只用于身份验证；岗位和账户范围仍由独立授权控制。</span></p></div></details>
