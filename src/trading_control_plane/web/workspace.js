...
async function renderApiAccess(credentialResult = null) {
  ...
  main.innerHTML = `<section class="page access-page api-access-page"><header class="page-head api-access-head"><div><p class="eyebrow">个人中心 · 开发者接入</p><h1>API 接入</h1><p class="lede">本页提供从创建 API Key 到接口调用的完整接入指南；字段以当前 OpenAPI 合同为唯一真源。</p></div><div class="api-access-actions"><span class="status-pill">${activeClients} 个可连接</span><a class="primary" href="#api-create-key">创建 API Key</a></div></header>
    <nav class="api-guide-nav" aria-label="API 接入指南"><a href="#api-create-key">创建 API Key</a><a href="#api-quickstart">快速开始</a><a href="#api-agent">调用示例</a><a href="#api-safety">安全边界</a><a href="#api-contract">接口目录</a></nav>
    <section class="api-credential-workspace" id="api-create-key" aria-labelledby="api-create-key-title"><header><div><p class="eyebrow">开始接入</p><h2 id="api-create-key-title">创建 API Key</h2><p>填写名称、使用范围和有效期即可创建。明文 Token 只显示一次，请立即保存。</p></div><span class="status-pill">${scopes.length} 个可用范围</span></header>${apiCredentialReveal(credentialResult)}${createForm}</section>
    ...
    <section class="api-guide-section" id="api-token" aria-labelledby="api-token-title"><div class="api-guide-index">02</div><div class="api-guide-content"><header><p class="eyebrow">凭证管理</p><h2 id="api-token-title">管理已有 API Keys</h2><p>API Key 明文只在创建或轮换成功时显示一次。页面此后只保留不可逆摘要、版本、有效期与最近使用时间。</p></header><div class="api-boundary-list compact"><p><b>权限</b><span>动态继承所属用户在当前 Team 的有效角色，不复制权限。</span></p><p><b>范围</b><span>只固定 Workspace / Team 上下文；Account / Venue 由用户当前 RBAC 逐资源校验。</span></p><p><b>生命周期</b><span>1–365 天，可停用、轮换或永久撤销；旧 Token 立即失效。</span></p></div><div class="section-heading api-client-heading"><div><p class="eyebrow">我的接入</p><h3>API Keys</h3></div><span class="subtle">截止 ${fmtDate(clientResponse.as_of)}</span></div><div class="member-access-list">${clientInventory}</div></div></section>
