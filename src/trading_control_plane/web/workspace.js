...
async function renderHome() {
  if (!hasCapability('operations.view')) {
    ...
    if (hasCapability('proposal.create')) {
      ...
      main.innerHTML = `<section class="page home-page"><article class="home-status tone-success"><div><p class="eyebrow">当前任务 · 提案发起人</p><h1>从机会开始形成交易判断</h1><p>查看机会、发起提案并跟踪自己的当前提案；不能审核、查看资金或操作交易任务。</p></div><div class="toolbar"><a class="primary" href="/opportunities" data-link>查看机会</a><a class="text-link" href="/proposals" data-link>查看提案记录 →</a></div></article><div class="stats home-stats"><div class="stat"><small>我的当前提案</small><b>${activeOwnedProposals.length}</b><span>草稿与等待审核</span></div><div class="stat"><small>等待独立审核</small><b>${pendingOwnedProposals.length}</b><span>创建者不能审核本人提案</span></div><div class="stat"><small>交易执行</small><b class="status-copy">未授权</b><span>提案批准后仍需实时风控</span></div><div class="stat"><small>真实下单</small><b class="status-copy">已关闭</b><span>本岗位无订单发送入口</span></div></div><section><div class="section-heading"><div><p class="eyebrow">下一步</p><h2>形成判断，不为操作而操作</h2></div></div><div class="home-priority-list"><a class="home-priority clear" href="/opportunities" data-link><span class="priority-number">1</span><div><small>市场观察</small><b>先核对机会来源与数据状态</b><p>只有数据当前、参数清楚且风险边界完整时才保存提案。</p></div><strong>查看机会</strong></a></div></section></section>`;
      return;
    }
  ...
  const shadowReviewCount = actionableReviews.filter(item => item.environment === 'SHADOW').length;
    ...
    : riskLimited
      ...
      : actionableReviews.length
        ? {
            ...
            copy:`当前没有运行阻断。生产 ${liveReviewCount} 笔，影子 ${shadowReviewCount} 笔；打开队列逐项判断，批准不会直接产生订单。`,
            href:'/reviews',
            action:'查看审核队列',
          }
  ...
  main.innerHTML = `<section class="page home-page"><article class="home-status tone-${safety.tone}"><div><p class="eyebrow">${safety.eyebrow}</p><h1>${escapeHtml(safety.title)}</h1><p>${escapeHtml(safety.copy)}</p></div><a class="primary" href="${safety.href}" data-link>${escapeHtml(safety.action)}</a></article>
    <div class="stats home-stats"><div class="stat"><small>运行告警</small><b class="${exceptions.length ? 'danger-text' : ''}">${exceptionCampaigns.size}</b><span>${exceptions.length ? `${exceptions.length} 项问题` : '无运行告警 / 无需处理'}</span></div><div class="stat"><small>独立审核待办</small><b class="${expiringReviews.length ? 'warning-text' : ''}">${actionableReviews.length}</b><span>${actionableReviews.length ? `生产 ${liveReviewCount} · 影子 ${shadowReviewCount}` : canReview ? '创建者不可审核自己的提案' : '当前身份不是审核人'}</span></div><div class="stat"><small>运行中交易任务</small><b>${activeCampaigns.length}</b><span>${activeCampaigns.length ? '保护与对账需持续有效' : '当前没有活动仓位流程'}</span></div><div class="stat"><small>风险边界</small><b class="${riskLimited ? 'warning-text status-copy' : 'status-copy'}">${escapeHtml(riskPolicySummary)}</b><span>真实下单${escapeHtml(liveOrderSendLabel)} · 自动加仓${escapeHtml(riskControl ? riskControlStatusLabel(riskControl.auto_add_gate.status) : '由管理员控制')}</span></div></div>
...
function renderScopeSetup() {
  ...
  main.innerHTML = `<section class="page scope-setup-page"><header class="page-head"><div><p class="eyebrow">Workspace → Team → Account</p><h1>${activeTeam ? escapeHtml(activeTeam.name) : '选择团队边界'}</h1><p class="lede">${escapeHtml(setupReason)}</p></div><span class="status-pill ${activeTeam?.trading_enabled ? 'status-APPROVED' : ''}">${activeTeam ? activeTeam.trading_enabled ? '已启用' : '安全配置中' : '尚未选择团队'}</span></header>
    ...
    ${activeTeam && !activeTeam.trading_enabled ? `<article class="home-status tone-attention"><div><p class="eyebrow">Fail closed</p><h2>团队业务能力尚未开放</h2><p>当前可配置成员、信号源、交易账户、通知和风险政策；生产交易链路保持关闭，交易模式由 Team 的服务端状态统一决定。</p></div><div class="toolbar">${hasCapability('access.manage') ? '<a class="secondary" href="/admin/users" data-link>配置团队成员</a>' : ''}${hasCapability('venue.view') ? '<a class="primary" href="/trading-mode" data-link>设置交易模式</a>' : '<span class="status-pill">由团队管理员处理</span>'}</div></article>` : ''}
    ${scopeCreationPanels()}
  </section>`;
...
function apiScopeOptions(scopes) {
  return scopes.map((scope, index) => `<option value="${index}">${escapeHtml(scope.workspace_name)} · ${escapeHtml(scope.team_name)}</option>`).join('');
}
...
4. Always distinguish LIVE from SHADOW. Never describe SHADOW results as real fills, positions, or capital.
...
async function renderApiAccess(credentialResult = null) {
  const [clientResponse, scopeResponse] = await Promise.all([api('/api/profile/api-keys'), api('/api/profile/api-key-contexts')]);
  ...
  const cards = clients.map(client => {
    ...
    return `<details class="member-access-card api-client-card ${client.token.status === 'ACTIVE' ? '' : 'is-inactive'}"><summary class="member-access-summary"><span class="member-summary-main"><b>${escapeHtml(client.name)}</b><small>${escapeHtml(client.workspace.name || 'Workspace')} · ${escapeHtml(client.team.name || 'Team')}</small><span class="member-role-tags">${roles}</span></span><span class="member-summary-actions"><span class="status-pill ${client.token.status === 'ACTIVE' ? 'status-APPROVED' : ''}">${escapeHtml(apiClientStatusLabel(client))}</span><strong>详情</strong></span></summary><div class="member-access-editor"><div class="agent-fact-strip"><span><b>Token 摘要</b><small>${escapeHtml(client.token.hint || '—')}</small></span><span><b>版本</b><small>${escapeHtml(client.token.version)}</small></span><span><b>到期</b><small>${fmtDate(client.token.expires_at)}</small></span><span><b>最近使用</b><small>${fmtDate(client.token.last_used_at)}</small></span></div><p class="microcopy"><b>权限真源：</b>所属用户当前业务 RBAC。每次请求按资源重新检查所属用户当前的 Team、Account、Venue 和角色；API Key 不保存独立权限或账户范围。当前状态：${escapeHtml(client.access_status)}。</p>${canOperate ? `<form class="api-client-test" data-test-api-client="${client.api_client_id}"><label>测试连接 Token<input name="token" type="password" autocomplete="off" placeholder="粘贴此 API Key" required></label><button class="secondary">测试连接</button><div class="form-error" role="alert"></div></form>` : ''}<div class="api-client-actions">${canOperate ? `<button class="secondary" type="button" data-toggle-api-client="${client.api_client_id}" data-active="${client.state === 'ACTIVE'}" data-version="${client.version}">${client.state === 'ACTIVE' ? '停用' : '重新启用'}</button><button class="text-button" type="button" data-rotate-api-client="${client.api_client_id}" data-token-version="${client.token.version}">轮换 Token</button><button class="danger" type="button" data-revoke-api-client="${client.api_client_id}" data-version="${client.version}">撤销</button>` : '<span class="subtle">该 API Key 已永久撤销</span>'}</div></div></details>`;
  }).join('');
  const createPanel = scopes.length ? `<details class="create-member-panel api-token-create" ${clients.length ? '' : 'open'}><summary><span><b>创建 API Key</b><small>选择 Workspace / Team；明文仅显示一次</small></span><strong>展开</strong></summary><form id="create-api-client-form" class="toolbox-content"><div class="field-grid"><label>API Key 名称<input name="name" pattern="[A-Za-z0-9._-]+" placeholder="例如 my-trading-bot" required></label><label>Workspace / Team<select name="scope" required>${apiScopeOptions(scopes)}</select></label><label>Token 有效天数<input name="expires_in_days" type="number" min="1" max="365" value="90" required></label></div><p class="microcopy">权限始终等于所属用户当前 RBAC；API Key 只固定 Workspace / Team 上下文，不设置独立 Account / Venue Scope。Token 可停用、轮换或永久撤销；资金、管理、风控恢复和需人工确认的动作仍须在网页完成。</p><div class="form-error" role="alert"></div><button class="primary">创建并显示一次 Token</button></form></details>` : '<article class="api-inline-notice tone-attention"><div><p class="eyebrow">暂无可用范围</p><h3>当前没有可创建 API Key 的团队上下文</h3><p>请联系团队管理员配置有效成员关系和业务角色。</p></div></article>';
  const clientInventory = cards || '<section class="empty-state api-client-empty"><div><h3>尚未创建 API Key</h3><p>选择 Workspace / Team 后创建；每次请求都使用你的当前业务 RBAC。</p></div></section>';
  const currentBaseUrl = location.origin;

  main.innerHTML = `<section class="page access-page api-access-page"><header class="page-head api-access-head"><div><p class="eyebrow">个人中心 · 开发者接入</p><h1>API 接入</h1><p class="lede">几分钟内完成认证、只读连接和范围校验。完整字段以当前 OpenAPI 合同为唯一真源。</p></div><span class="status-pill">${activeClients} 个可连接</span></header>
    ...
    <section class="api-guide-section" id="api-token" aria-labelledby="api-token-title"><div class="api-guide-index">02</div><div class="api-guide-content"><header><p class="eyebrow">API Key</p><h2 id="api-token-title">创建用户凭证</h2><p>API Key 明文只在创建或轮换成功时显示一次。页面此后只保留不可逆摘要、版本、有效期与最近使用时间。</p></header><div class="api-boundary-list compact"><p><b>权限</b><span>动态继承所属用户在当前 Team 的有效角色，不复制权限。</span></p><p><b>范围</b><span>只固定 Workspace / Team 上下文；Account / Venue 由用户当前 RBAC 逐资源校验。</span></p><p><b>生命周期</b><span>1–365 天，可停用、轮换或永久撤销；旧 Token 立即失效。</span></p></div>${apiCredentialReveal(credentialResult)}${createPanel}<div class="section-heading api-client-heading"><div><p class="eyebrow">我的接入</p><h3>API Keys</h3></div><span class="subtle">截止 ${fmtDate(clientResponse.as_of)}</span></div><div class="member-access-list">${clientInventory}</div></div></section>
    ...
    <section class="api-guide-section" id="api-safety" aria-labelledby="api-safety-title"><div class="api-guide-index">04</div><div class="api-guide-content"><header><p class="eyebrow">安全边界</p><h2 id="api-safety-title">客户端提示不能越过服务端控制</h2><p>API Key 与所属用户是同一审核主体。服务端权限、独立审核、风险政策、数据时效和危险 Gate 始终是最终边界。</p></header><div class="api-boundary-list"><p><b>只读默认</b><span>从 GET 接口开始；写操作必须得到明确授权，并按当前 OpenAPI 请求体提供唯一 <code translate="no">idempotency_key</code>。</span><strong class="status-pill status-APPROVED">默认安全</strong></p><p><b translate="no">LIVE / SHADOW</b><span>环境字段必须显式读取和保留；<code translate="no">SHADOW</code> 结果不得描述成真实成交、持仓或资金。</span><strong class="status-pill">文字区分</strong></p><p><b>数据时效</b><span>检查 <code translate="no">as_of</code>、<code translate="no">observed_at</code>、<code translate="no">fetched_at</code> 与 <code translate="no">data_status</code>。缺失、过期或限流不等于实时，也不等于 0。</span><strong class="status-pill">失败即阻断</strong></p><p><b>危险能力</b><span><code translate="no">LIVE_ORDER_SEND</code>、<code translate="no">CAPITAL_TRANSFER</code>、自动加仓和资金自动化 Gate 默认关闭；不处理密码、交易所密钥、签名材料或广播秘密。</span><strong class="status-pill">默认关闭</strong></p></div></div></section>
    <section class="api-guide-section" id="api-contract" aria-labelledby="api-contract-title"><div class="api-guide-index">05</div><div class="api-guide-content"><header><p class="eyebrow">完整接口</p><h2 id="api-contract-title">OpenAPI 是唯一接口合同</h2><p>指南只维护接入流程和安全原则，不重复维护全部请求字段。字段、枚举、查询参数和响应模型以运行中的 OpenAPI 为准。</p></header><div class="api-contract-actions"><a class="primary" href="/openapi.json" target="_blank" rel="noopener">打开 OpenAPI JSON</a><a class="secondary" href="/docs/API_KEY_QUICKSTART.md" target="_blank" rel="noopener">查看完整接入指南</a></div><div class="table-wrap is-scrollable api-endpoint-table"><table><thead><tr><th>常用只读接口</th><th>用途</th><th>要求</th></tr></thead><tbody><tr><td><code translate="no">GET /api/api-key/connection</code></td><td>验证 API Key、动态角色和 Team 上下文</td><td>有效 Token</td></tr><tr><td><code translate="no">GET /api/instruments</code></td><td>读取当前团队可见的合约目录</td><td>已认证</td></tr><tr><td><code translate="no">GET /api/opportunities</code></td><td>读取带来源与时效状态的机会快照</td><td><code translate="no">opportunity.view</code></td></tr><tr><td><code translate="no">GET /api/proposals</code></td><td>读取当前范围提案</td><td><code translate="no">proposal.view</code></td></tr><tr><td><code translate="no">GET /api/campaigns</code></td><td>读取交易任务</td><td><code translate="no">operations.view</code></td></tr><tr><td><code translate="no">GET /api/results?environment=SHADOW</code></td><td>按环境读取实际结果</td><td><code translate="no">results.view</code></td></tr><tr><td><code translate="no">GET /api/audit?environment=SHADOW&amp;limit=200</code></td><td>按环境读取审计时间线</td><td><code translate="no">results.view</code></td></tr></tbody></table></div><p class="microcopy">当前仅部分列表接口提供 <code translate="no">limit</code>：通知为 1–200，审计为 1–500。不要自行假设 <code translate="no">cursor</code>、<code translate="no">offset</code> 或 <code translate="no">page</code> 参数；以 OpenAPI 为准。</p></div></section>
  </section>`;
  ...
  document.querySelector('#create-api-client-form')?.addEventListener('submit', async event => { event.preventDefault(); const form = event.currentTarget; const data = new FormData(form); const scope = scopes[Number(data.get('scope'))]; if (!scope) return; const payload = {name:data.get('name'), workspace_id:scope.workspace_id, team_id:scope.team_id, expires_in_days:Number(data.get('expires_in_days')), idempotency_key:crypto.randomUUID()}; await withPending(event.submitter, '创建中…', async () => { try { const response = await api('/api/profile/api-keys', {method:'POST', body:JSON.stringify(payload)}); await renderApiAccess(response.result); enhanceRenderedPage(); } catch (error) { showApiError(error, form.querySelector('.form-error')); } }); });
