...
async function renderNotifications() {
  ...
  const createForm = data.can_manage ? `<details class="card notification-route-create"><summary><span><b>新建团队通知路由</b><small>测试与生产路由按环境隔离，渠道凭据独立加密保存</small></span><strong>展开</strong></summary><form id="notification-route-create-form" class="toolbox-content" data-version="0" data-channel="TELEGRAM"><div class="field-grid"><label>环境<select name="environment"><option value="TESTNET" ${defaultEnvironment === 'TESTNET' ? 'selected' : ''}>测试模式</option><option value="LIVE" ${defaultEnvironment === 'LIVE' ? 'selected' : ''}>生产模式</option></select></label><label>路由名称<input name="name" maxlength="120" required placeholder="例如 提案审核群"></label><label>通知渠道<select name="channel"><option value="TELEGRAM">Telegram</option><option value="SLACK">Slack</option><option value="LARK">飞书 / Lark</option><option value="EMAIL">邮件</option></select></label><label>路由状态<select name="enabled"><option value="true">启用</option><option value="false">停用</option></select></label></div><fieldset><legend>订阅事件</legend><div class="notification-event-grid">${notificationEventOptions(data.event_catalog, defaultEvents)}</div></fieldset><fieldset><legend>加密渠道配置</legend><div class="field-grid" data-notification-create-config>${notificationConfigurationFields('TELEGRAM', {required:true})}</div></fieldset><p class="safety-note">保存只建立通知路由。通知进程没有交易、资金、签名或广播接口；API 和页面只返回脱敏目的地。</p><div class="form-error" role="alert"></div><div class="form-actions"><button class="primary" type="submit">加密保存路由</button></div></form></details>` : '';
