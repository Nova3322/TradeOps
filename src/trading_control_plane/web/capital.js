...
function renderDirectCapitalConfigurationEditor(directConfiguration, selectedTreasuryProvider) {
  ...
  return `<details class="card direct-capital-config-editor">
    ...
    <form id="direct-capital-config-form" class="toolbox-content compact-form">
      ...
      <section class="capital-config-section common-capital-fields"><h3>2. 共用账户与安全边界</h3><p>两种链上金库共用币安、Hyperliquid、自有地址和金额限制；新操作只引用当前使用的金库。</p><div class="field-grid"><label>授权自有 Arbitrum 地址<span class="field-help">用于 Arbitrum One 原生 USDC 充值和接收资金的钱包地址。钱包需准备少量 ETH 支付链上 Gas。</span><input name="owned_arbitrum_address" autocomplete="off" placeholder="${configuredPlaceholder(directConfiguration.owned_arbitrum_address_configured)}"></label><label>币安默认账户<input name="binance_account_id" autocomplete="off" placeholder="${configuredPlaceholder(directConfiguration.binance_account_configured)}"></label><label>币安白名单入金地址<input name="binance_deposit_address" autocomplete="off" placeholder="${configuredPlaceholder(directConfiguration.binance_whitelist_destination_configured)}"></label><label>币安受限提现地址<input name="binance_withdrawal_address" autocomplete="off" placeholder="${configuredPlaceholder(directConfiguration.binance_withdrawal_destination_configured)}"><small data-withdrawal-scope-help></small></label><label>Hyperliquid 默认账户<input name="hyperliquid_account_id" autocomplete="off" placeholder="${configuredPlaceholder(directConfiguration.hyperliquid_account_configured)}"></label><label>Hyperliquid 充值桥地址（Bridge）<span class="field-help">仅用于 Arbitrum One 原生 USDC。生产环境必须使用 Hyperliquid 官方 Bridge2 地址。</span><input name="hyperliquid_bridge_address" autocomplete="off" placeholder="${configuredPlaceholder(directConfiguration.hyperliquid_contract_configured)}"></label><label>单次金额上限（USDC）<input name="max_amount" type="number" step="any" min="0.000001" placeholder="留空保持当前值"></label><label>最大费用上限（USDC）<span class="field-help">单笔资金划转允许的最高 USDC 手续费。当前费用超过此上限时，系统将阻止划转；该费用由交易所或资金通道收取，不支付给 TradeOps。</span><input name="max_fee" type="number" step="any" min="0" placeholder="留空保持当前值"><small>最大费用上限必须低于单次金额上限。</small></label></div></section>
