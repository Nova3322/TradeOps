...
async function renderCapitalCenter() {
  ...
  const directRows = (item.direct_operations || []).map(operation => {
    ...
    const operationActions = `${startButton}${nextActionButton}${treasuryWalletButton}${notiltExecutionWalletButton}${notiltDestinationWalletButton}${hyperliquidWalletButton}${binanceSubmitButton}${notiltReceiptButton}${safeWithdrawalReceiptButton}${binanceReceiptButton}${hyperliquidReceiptButton}${treasuryReceiptButton}`;
    const continuationReady = operationActions && operation.status !== 'BLOCKED' && blockerCodes.length === 0;
    if (continuationReady) directContinuationCards.push(`<article class="capital-transfer-continuation"><div><small>${shortId(operation.operation_id)} · ${escapeHtml(operation.treasury_provider === 'SAFE_SPENDING_LIMIT' ? 'Safe Spending Limits' : 'NoTilt Vault')}</small><b>${escapeHtml(label)}</b><span>${fmtNumber(operation.amount)} ${escapeHtml(operation.asset)} · ${escapeHtml(fmtStatus(operation.status))}</span></div><div class="capital-transfer-continuation-actions">${operationActions}</div></article>`);
  ...
  const directContinuations = directContinuationCards.length ? `<section class="capital-transfer-continuations" aria-label="待继续划转"><div class="capital-transfer-continuations-heading"><div><p class="eyebrow">待继续划转</p><h3>在划转区完成下一步</h3></div><span>${directContinuationCards.length} 项</span></div><div class="capital-transfer-continuation-list">${directContinuationCards.join('')}</div></section>` : '';
