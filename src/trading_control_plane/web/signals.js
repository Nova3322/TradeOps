...
function webhookSignalCard(item, canPropose) {
  ...
  const action = item.proposal
    ...
    : canPropose && item.proposal_eligibility === 'ELIGIBLE' && options
      ? `<details class="operation-toolbox"><summary><span><b>手动创建冻结提案</b><small>仍需独立审核、服务端风控与交易授权</small></span><strong>展开</strong></summary><form class="toolbox-content signal-proposal-form" data-signal-event-id="${escapeHtml(item.signal_event_id)}"><div class="field-grid"><label>账户<input name="account_id" required></label><label>合约<select name="instrument_id">${options}</select></label><label>环境<select name="environment"><option value="SHADOW">影子</option><option value="TESTNET">测试网</option><option value="LIVE">实盘</option></select></label><label>风险档位<select name="risk_tier"><option value="LOW">低</option><option value="MEDIUM">中</option><option value="HIGH">高</option></select></label><label>数量<input name="quantity" type="number" step="any" min="0.000000000000000001" required></label><label>最大风险<input name="max_risk" type="number" step="any" min="0.000000000000000001" required></label><label>有效时间（分钟）<input name="expires_in_minutes" type="number" min="480" max="1440" value="480" required></label></div><label>人工判断理由<textarea name="rationale" minlength="10" rows="3" required></textarea></label><div class="form-error" role="alert"></div><div class="form-actions"><button class="primary">创建并提交独立审核</button></div></form></details>`
      : `<p class="safety-note">${escapeHtml(blocker === '无' ? '服务端未开放当前信号的提案操作。' : blocker)} 信号记录仍保留且不会触发自动审核或下单。</p>`;
  return `<article class="card webhook-signal-card" data-signal-direction="${escapeHtml(item.direction)}" data-signal-event-id="${escapeHtml(item.signal_event_id)}" data-signal-source-id="${escapeHtml(item.signal_source_id)}"><div class="card-heading"><div><p class="eyebrow">${escapeHtml(sourceName)} · ${escapeHtml(item.provider)}</p><h2>${escapeHtml(item.symbol)} · <span class="${directionClass}">${escapeHtml(fmtDirection(item.direction))}</span></h2></div><span class="status-pill ${item.proposal_eligibility === 'ELIGIBLE' ? 'status-APPROVED' : item.proposal ? 'status-PENDING_REVIEW' : 'status-REJECTED'}">${escapeHtml(proposalStatus)}</span></div><dl class="webhook-signal-facts">${definition('来源', `${sourceName}${item.signal_source_deleted ? '（已删除）' : ''}`)}${definition('策略', `${item.strategy_id} · ${item.strategy_version}`)}${definition('交易所', item.venue)}${definition('币种', item.symbol)}<div><dt>方向</dt><dd class="${directionClass}">${escapeHtml(fmtDirection(item.direction))}</dd></div>${definition('周期', item.timeframe || '未提供')}${definition('价格', item.reference_price == null ? '未提供' : fmtNumber(item.reference_price))}${definition('发生时间', fmtDate(item.occurred_at))}${definition('接收时间', fmtDate(item.received_at))}${definition('新鲜度', freshness)}${definition('阻断原因', blocker)}${definition('提案状态', proposalStatus)}</dl>${action}</article>`;
}
...
function opportunityCard(item) {
  ...
  const activeProposalCopy = activeProposal
    ...
    : '';
  return `<article class="card" data-opportunity-card="${escapeHtml(cardId)}" data-opportunity-direction="${escapeHtml(item.direction)}" data-opportunity-state="${viewState}"><div class="card-top"><div><span class="subtle">${escapeHtml(fmtVenueLabel(item.venue))} · ${escapeHtml(timeframes.join(' / '))}</span><div class="symbol">${escapeHtml(item.symbol)}</div></div><span class="tag ${directionClass}">${escapeHtml(fmtDirection(item.direction))}</span></div>
