...
function signalSourceStatus(source) {
  ...
  if (source.mode === 'PERPTAPE' && ['STALE','WAITING','NOT_CONFIGURED'].includes(source.perptape?.data_status)) return {label:'等待恢复', tone:'status-PENDING_REVIEW'};
...
async function renderSignalSources() {
  ...
  const credentialReveal = reveal ? `<article class="card signal-credential-reveal" role="status"><div><p class="eyebrow">仅显示一次</p><h2>${escapeHtml(reveal.name)} 的新${reveal.mode === 'WEBHOOK' ? '签名密钥' : ' API Key'}</h2><p>立即保存到受控密钥库；关闭或刷新后，TradeOps 不再回显。</p><code data-one-time-secret>${escapeHtml(reveal.secret)}</code></div><div class="toolbar"><button class="secondary" data-copy-one-time-secret>复制</button><button class="secondary" data-dismiss-one-time-secret>我已保存</button></div></article>` : '';
  const cards = sources.map(signalSourceCard).join('');
  main.innerHTML = `<section class="page signal-sources-page"><header class="page-head"><div><p class="eyebrow">空间隔离 · ${sources.length} 个接入</p><h1>信号源设置</h1><p class="lede">配置当前空间的 Perptape 和独立 Webhook 接入。实时信号分别在 Perptape 与 Webhook 页面展示，不在设置页聚合或混排。</p></div><div class="toolbar"><button class="secondary" data-refresh>刷新状态</button></div></header>${credentialReveal}<details class="signal-source-catalog"><summary><span><span class="eyebrow">接入状态</span><strong>当前接入的信号源</strong><small>${sources.length} 个接入 · 截止 ${fmtDate(status.as_of)}</small></span><b><span data-catalog-collapsed>查看详情</span><span data-catalog-expanded>收起详情</span></b></summary><div class="signal-source-catalog-content">${cards ? `<div class="signal-source-grid">${cards}</div>` : '<div class="callout tone-attention"><b>安全保护：</b>当前空间尚无信号源，其他空间或部署默认值不会被当作当前配置。</div>'}</div></details>${canManage ? signalSourceCreatePanel(sources.some(item => item.mode === 'PERPTAPE')) : '<div class="callout"><b>只读：</b> 当前身份可以查看接入状态，但没有信号源管理权限。</div>'}</section>`;
...
function renderOpportunitySnapshot(result, sourceError = null, preservedFilters = {}) {
  ...
  const transportNotice = transportState === 'WEBSOCKET_FAILED' || transportState === 'POLLING_FAILED'
      ? '<article class="source-status tone-danger"><div><p class="eyebrow">安全保护</p><h2>连续信号接入未通过</h2><p>仅保留可辨识的旧快照；过期或缺失事实不会创建新提案。</p></div></article>'
      : '';
  main.innerHTML = `<section class="page" data-opportunity-snapshot="${escapeHtml(result?.snapshot_id || '')}"><header class="page-head"><div><p class="eyebrow">Perptape · ${escapeHtml(localizedText(transportLabel))}</p><h1>${upstreamLive ? '实时机会' : '机会快照'}</h1><p class="lede">${upstreamLive ? '实时流' : '当前快照'}汇总同一币对、同一方向的多个突破周期。同一轮卡片、状态和按钮都来自同一个服务端事实快照；创建时仍会重新校验。</p></div><div class="opportunity-head-tools"><div class="opportunity-tools"><span>信号快照 ${fmtDate(result?.snapshot_generated_at || result?.as_of)}</span>${canPropose ? `<a class="secondary" href="/opportunities/defaults" data-link>默认配置${proposalDefaults.configured ? '' : ' · 未完成'}</a>` : ''}</div><div class="toolbar"><span class="status-pill" data-live-status>${sourceError ? '页面连接已中断，正在重连' : '页面正在连接'}</span>${canPropose ? '<a class="primary" href="/proposals/new" data-link>＋ 人工提案</a>' : ''}<button class="secondary" data-refresh>刷新机会</button></div></div></header>
    ${transportNotice}
...
function proposalDefaultEditor(currentDefault, accounts) {
  const expiryHours = currentDefault ? Number(currentDefault.expires_in_minutes) / 60 : 8;
  const compactDecimal = (value, fallback) => {
    const raw = String(value ?? fallback);
    return raw.includes('.') ? raw.replace(/0+$/, '').replace(/\.$/, '') || '0' : raw;
  };
  const liveAccounts = (accounts || []).filter(item => item.active && item.environment === 'LIVE');
  const accountOptions = liveAccounts.length
    ? liveAccounts.map(item => `<option value="${escapeHtml(item.account_id)}" ${item.account_id === currentDefault?.account_id ? 'selected' : ''}>${escapeHtml(`${item.label} · ${fmtVenueLabel(item.venue)} · ${item.account_id}`)}</option>`).join('')
    : '<option value="">尚无可用生产账户</option>';
  return `<form id="proposal-default-form" class="form-panel proposal-default-form"><div class="field-grid"><label>默认账户<span class="field-help">选择不可变账户 ID；其他交易所仅在唯一精确账户时自动匹配</span><select name="account_id" required ${liveAccounts.length ? '' : 'disabled'}>${accountOptions}</select></label><label>风险档位<select name="risk_tier"><option value="LOW" ${currentDefault?.risk_tier === 'LOW' ? 'selected' : ''}>低</option><option value="MEDIUM" ${!currentDefault || currentDefault.risk_tier === 'MEDIUM' ? 'selected' : ''}>中</option><option value="HIGH" ${currentDefault?.risk_tier === 'HIGH' ? 'selected' : ''}>高</option></select></label><label>名义仓位（USDT）<input name="notional" type="number" step="any" min="0" value="${escapeHtml(compactDecimal(currentDefault?.notional, '100'))}" required></label><label>最大风险<input name="max_risk" type="number" step="any" min="0" value="${escapeHtml(compactDecimal(currentDefault?.max_risk, '1'))}" required></label><label>失效距离（基点）<input name="invalidation_bps" type="number" min="1" max="5000" value="${escapeHtml(currentDefault?.invalidation_bps || '200')}" required></label><label>有效时间（小时，至少 8 小时）<input name="expires_in_hours" type="number" min="8" max="24" value="${escapeHtml(Math.max(8, expiryHours))}" required></label><label>自动提案阈值<select name="auto_proposal_min_timeframes"><option value="3" ${currentDefault?.auto_proposal_min_timeframes !== 4 ? 'selected' : ''}>至少 3 个不同周期</option><option value="4" ${currentDefault?.auto_proposal_min_timeframes === 4 ? 'selected' : ''}>4 个不同周期</option></select></label><label class="checkbox-row"><input name="auto_proposal_enabled" type="checkbox" ${currentDefault?.auto_proposal_enabled ? 'checked' : ''}><span>自动创建多周期冻结待审核提案</span></label></div><label>默认理由<textarea name="rationale" rows="3" required>${escapeHtml(currentDefault?.rationale || '使用管理员保存的一键创建默认配置，仅创建待审核提案。')}</textarea></label><p class="safety-note">提案至少保留 8 小时。自动提案只会进入待审核队列；不会自动审核、授权、创建订单或下单。阈值变更只影响之后观察到的新鲜信号。</p><div class="form-error" role="alert"></div><div class="form-actions"><a class="secondary" href="/opportunities" data-link>取消</a><button class="primary" ${liveAccounts.length ? '' : 'disabled'}>保存新版本</button></div></form>`;
}
