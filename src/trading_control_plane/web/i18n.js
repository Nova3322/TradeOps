...
const ENGLISH_EXACT = new Map(Object.entries({
  ...
  '本页提供从创建 API Key 到接口调用的完整接入指南；字段以当前 OpenAPI 合同为唯一真源。':'This page is the complete guide from API Key creation through API calls; the current OpenAPI contract is the sole source of truth.',
  ...
  '完整接口':'Complete API', 'OpenAPI 是唯一接口合同':'OpenAPI is the sole API contract', '本页即完整接入指南':'This page is the complete access guide', '打开 OpenAPI JSON':'Open OpenAPI JSON', '查看完整接入指南':'Open the complete access guide',
  '本页维护创建凭证、验证连接、调用示例和安全原则；字段、枚举、查询参数和响应模型以运行中的 OpenAPI 为准。':'This page covers credential creation, connection validation, usage examples, and safety principles. Use the running OpenAPI for fields, enums, parameters, and response models.',
