...
const ENGLISH_EXACT = new Map(Object.entries({
  ...
  '账户 ID':'Account ID', '显示名称':'Display name', '账户名称':'Account name', '保存名称':'Save name', '更新 / 轮换凭据':'Update / rotate credentials', '编辑与凭据':'Edit and credentials', '敏感字段只用于轮换，永不回显':'Sensitive fields are used only for rotation and are never displayed', '凭据':'Credentials', '最近验证':'Latest verification', '尚未验证':'Not verified', '交易状态':'Trading status', '运行同步':'Runtime sync', '未绑定':'Not bound', '连接测试':'Test connection',
  ...
  '账户地址':'Account address', 'API Wallet 地址（可选）':'API Wallet address (optional)', 'API Wallet 私钥（配置交易身份时必填）':'API Wallet private key (required for trading identity)', '加密保存且不会回显':'Encrypted and never displayed', '金库编号':'Vault ID', '金库地址':'Vault address', '币安默认账户':'Default Binance account', '币安白名单入金地址':'Binance allowlisted deposit address', '币安受限提现地址':'Binance restricted withdrawal address', '默认账户':'Default account', '单次金额上限（USDC）':'Per-operation limit (USDC)', '最大费用上限（USDC）':'Maximum fee (USDC)', '必须低于单次金额上限':'Must be lower than the per-operation limit',
  '保存的是不可变账户 ID；显示名称仅用于识别，避免名称与 ID 混淆。':'The immutable account ID is saved; the display name is only for identification.', '当前保存值与生产运行不一致，请重新选择':'The saved value does not match the Production runtime. Select again.', '请选择账户管理中的生产默认账户':'Select the Production default from Account management.', '生产运行尚未绑定默认账户':'The Production runtime has no default account binding.', '交易所账户从账户管理中的生产配置选择，并始终保存精确账户 ID。':'Select exchange accounts from Production configurations in Account management; the exact account ID is always saved.',
  ...
  '请从账户管理的生产账户下拉列表重新选择。系统校验不可变账户 ID，不使用可编辑的显示名称。':'Select the Production account again from the Account management list. Validation uses the immutable account ID, not the editable display name.',
