...
const ENGLISH_EXACT = new Map(Object.entries({
  ...
  '安全':'Security', '修改登录密码':'Change sign-in password', '展开':'Expand', '当前密码':'Current password', '当前账户密码':'Current account password', '新密码':'New password', '确认新密码':'Confirm new password',
  ...
  '只读用户':'Observer', '提案发起人':'Proposer', '审核人':'Reviewer', '风险管理人员':'Risk manager', '风险管理':'Risk management',
  ...
  '个人中心 · 开发者接入':'Personal center · Developer access',
  '创建 API Key，验证连接，并按示例快速完成接口接入。完整字段请查看 OpenAPI。':'Create an API Key, validate the connection, and use the examples to integrate quickly. See OpenAPI for complete fields.',
  'API 接入指南':'API access guide', '接口目录':'Endpoint catalog', '开始接入':'Get started', '个可用范围':'available scopes',
  '填写名称、使用范围和有效期即可创建。明文 Token 只显示一次，请立即保存。':'Choose a name, scope, and lifetime to create an API Key. The plaintext token is shown once; save it immediately.',
  ...
  '成功条件':'Success criteria', '创建用户凭证':'Create a user credential', '凭证管理':'Credential management', '管理已有 API Keys':'Manage existing API Keys', '权限':'Permissions', '范围':'Scope', '生命周期':'Lifecycle',
  ...
  '安全边界':'Safety boundaries', '所有调用都遵循账户安全规则':'Every API call follows account safety rules',
  'API Key 继承当前账号权限。独立审核、风险政策、数据时效和高风险操作限制始终生效。':'The API Key inherits the current account permissions. Independent review, risk policy, freshness, and high-risk operation restrictions always apply.',
  '接口文档':'API documentation', '接口说明与字段定义':'Endpoint descriptions and field definitions', '打开 OpenAPI JSON':'Open OpenAPI JSON',
  '常用接口列于下方；完整字段、枚举、查询参数和响应结构请查看 OpenAPI。':'Common endpoints are listed below. See OpenAPI for complete fields, enums, query parameters, and response structures.',
  '常用只读接口':'Common read-only endpoint', '用途':'Purpose', '要求':'Requirement', '我的接入':'My API Keys',
  '创建 API Key':'Create API Key', 'API Key 名称':'API Key name', 'Token 有效天数':'Credential lifetime in days',
  '创建并显示一次 API Key':'Create and display API Key once', '尚未创建 API Key':'No API Key has been created', '打开上方创建区域，即可为当前工作区创建第一个 API Key。':'Open the creation panel above to create the first API Key for the current workspace.',
  ...
  '权限来源：':'Permission source: ', '继承当前账号在该团队的有效权限。每次请求都会重新检查团队、账户、交易所和岗位范围；API Key 不保存额外权限。访问状态：':'Inherits the current account permissions in this team. Every request rechecks team, account, venue, and role scope. The API Key stores no additional permissions. Access status: ',
  '可用':'Available', '工作区访问已停用':'Workspace access disabled', '团队访问已停用':'Team access disabled', '当前没有有效权限':'No current permission', '需要重新确认访问权限':'Access needs to be reconfirmed',
  ...
  '创建或轮换时，明文 API Key 只显示一次；之后仅显示摘要、版本、有效期与最近使用时间。':'The plaintext API Key is displayed once on creation or rotation. Only its hint, version, expiry, and last-used time are shown afterward.',
  ...
  'API Key 将绑定当前工作区和团队，并继承当前账号的有效权限。如需为其他工作区创建，请先切换到对应工作区。API Key 可随时停用、轮换或永久撤销；资金、管理、风险恢复和需人工确认的操作仍须在网页完成。':'The API Key is bound to the current workspace and team and inherits the account permissions. Switch workspaces first to create one elsewhere. It can be disabled, rotated, or revoked at any time; capital, administration, risk restoration, and confirmation-required operations remain on the web.',
  '当前工作区和团队':'Current workspace and team', '范围已确定':'Scope confirmed', '等待选择工作区':'Waiting for workspace', '切换工作区':'Switch workspace',
  '需要选择工作区':'Workspace required', '请先进入要接入的工作区和团队':'Enter the workspace and team you want to connect first', '切换完成后，即可为当前工作区创建 API Key。':'After switching, you can create an API Key for the current workspace.', '仅显示当前账号创建的 API Keys':'Only API Keys created by the current account are shown',
  ...
  '只读默认':'Read-only by default', '文字区分':'Explicit labeling', '安全保护':'Safety protection', '失败即阻断':'Fail closed', '默认安全':'Safe by default', '危险能力':'Dangerous capabilities', '数据时效':'Data freshness',
  ...
  '数据库检查中':'Checking database', '数据库已连接':'Database connected', '数据库未连接':'Database disconnected', '数据库状态未知':'Database status unknown', '数据缺失时自动阻止交易':'Missing data blocks trading automatically', '跳到主要内容':'Skip to main content',
  ...
  '取消并关闭':'Cancel and close', '确认并继续':'Confirm and continue', '用于本次操作的身份二次验证；不会保存。':'Used only to re-authenticate this action; it is not saved.', '权限范围':'Permission scope',
  ...
  '批准这份冻结提案？':'Approve this frozen proposal?', '确认批准':'Confirm approval', '验证身份并批准':'Verify identity and approve',
  ...
  '系统将记录你的一次独立批准。达到所需审批票数后会自动运行风控；不会签发交易授权、创建订单或下单。':'The system records one independent approval. Once the required approval count is reached, it automatically runs the risk check; it does not issue authorization, create an order, or place a trade.',
  ...
  '当前任务 · 审核员':'Current tasks · Reviewer', '只处理非本人提案与风险恢复申请；不能发起提案、查看资金或操作交易任务。':'Handle only independent proposal reviews and risk-restoration requests. You cannot create proposals, view capital, or operate trades.', '只处理非本人提案与风险恢复申请；不能发起提案、查看资金或手工操作交易任务。':'Handle only independent proposal reviews and risk-restoration requests. You cannot create proposals, view capital, or manually operate trades.',
  '提案审核':'Proposal reviews', '非本人 · 未投票 · 未到期':'Other authors · not voted · active', '当前没有审核待办':'No proposal reviews are pending',
  '工作范围':'Role scope', '独立判断':'Independent decision', '生产与测试始终分开标记':'Production and Testnet remain explicitly separated', '只读执行':'Read-only execution', '批准不下单 · 不能自审':'Approval never sends an order · self-review is blocked', '审核后自动执行':'Automatic execution after review', '不能自审 · 实时风控失败即阻断':'No self-review · live-risk failure blocks execution',
  ...
  '只包含未投票、未到期的当前提案；审核达标并通过实时风控后会自动执行。':'Only active proposals you have not voted on are included. Execution starts automatically after the review threshold and live-risk checks pass.',
  ...
  '审核队列已清空':'The review queue is clear', '自己的提案、已投票、已过期或已结束记录不会出现在待办中。':'Your own proposals and voted, expired, or completed records are excluded from current tasks.', '暂无待办':'Nothing pending', '无需操作':'No action needed',
  ...
  '批准不会自动下单。风险管理需要按当前账户事实重新风控、签发短期授权，再创建交易任务。':'Approval never places an order automatically. Risk management must rerun risk checks against current account facts, issue short-lived authorization, and then create a trade.',
  '批准后系统会自动运行风控。':'The system automatically runs the risk check after approval.', '达到所需票数后系统会自动运行风控。':'The system automatically runs the risk check once the required approvals are recorded.',
  '系统正在运行风险检查':'The system is running the risk check', '审核已完成，系统会基于最新账户与受管资金事实自动冻结风控结果。':'Review is complete. The system automatically records a risk result from the latest account and managed-capital facts.',
  '系统风控处理中':'System risk check in progress', '达到审批阈值后由系统自动读取最新仓位、权益、受管资金、保护和总风险容量。':'After the approval threshold is met, the system automatically reads the latest position, equity, managed-capital, protection, and total-risk-capacity facts.',
  '达到审批阈值后系统会自动运行风险检查。':'The system automatically runs the risk check after the approval threshold is met.',
  '审批达标后系统自动运行实时风控；通过后再签发短期授权，缺少事实或安全开关未满足时仍会阻断。':'After approval, the system automatically runs live risk checks. Short-lived authorization comes only after a pass; missing facts or closed safety gates still block progress.',
  '达到审核阈值后系统会按当前账户事实自动风控、签发短期授权、预留风险并创建交易任务。':'After the approval threshold is met, the system automatically evaluates live risk, issues a short-lived authorization, reserves risk, and creates the trade.',
  '系统会自动继续风控、授权、风险预留与任务创建；缺少事实或安全开关未满足时会安全阻断，不需要人工点击下一步。':'The system automatically continues through risk, authorization, reservation, and trade creation. Missing facts or closed safety gates block safely without another manual click.',
  '自动流程处理中':'Automatic workflow in progress',
  ...
  'Perptape · HTTPS 轮询回退':'Perptape · HTTPS polling fallback',
  ...
  '待我审核':'Assigned to me', '搜索标的':'Search instrument', '全部方向':'All directions', '来源':'Source', '全部来源':'All sources', '系统机会':'System opportunity', '人工判断':'Manual thesis', '个结果':'results', '显示更多':'Show more', '已显示全部':'All shown', '查看详情':'View details', '返回审核队列':'Back to review queue', '批准提案':'Approve proposal', '拒绝提案':'Reject proposal', '提案记录分页':'Proposal record pagination',
  ...
  '批准或拒绝前都需要再次确认；不会直接下单。':'Approval or rejection requires final confirmation; neither places an order.', '批准是最后一个常规人工节点；达到审核阈值并通过实时风控后，系统会自动完成生产交易。':'Approval is the final normal manual step. Once the threshold and live-risk checks pass, the production trade completes automatically.', '系统允许开多少':'How much the system may open', '等待审核通过':'Waiting for approval',
  ...
  '交易任务已进入自动执行':'Trade entered automatic execution', '该提案不能再创建第二个初仓意图；Freqtrade 发送、成交确认、Facts 和异常处理统一在交易任务中自动推进。':'A second initial-entry intent cannot be created. Freqtrade sending, fill confirmation, Facts, and exception handling advance automatically in this trade.',
  '系统正在签发短期授权':'Issuing short-lived authorization', '风险检查已通过；系统正在自动签发限时、限数量、限风险的交易授权。':'Risk passed. The system is automatically issuing a time-, quantity-, and risk-bounded authorization.',
  '系统正在创建交易任务':'Creating trade automatically', '系统正在自动预留风险并创建唯一初仓意图；随后由受控执行进程在 Gate、租约和幂等边界内推进。':'The system is automatically reserving risk and creating the unique initial-entry intent, then a controlled executor advances it within Gate, lease, and idempotency boundaries.',
  '无需人工点击；系统会从当前持久状态继续推进。':'No manual click is required; the system continues from durable state.',
  '系统正在读取最新事实、预留风险并创建唯一初仓意图。':'The system is reading current facts, reserving risk, and creating the unique initial-entry intent.',
  ...
  '统一汇总当前团队内需要你独立判断、尚未到期且尚未投票的提案。生产与测试环境始终分开标记；批准不等于下单。':'This inbox combines current-team proposals that still need your independent decision, have not expired, and have not received your vote. Production and Testnet stay distinctly labelled; approval never places an order.', '统一汇总当前团队内需要你独立判断、尚未到期且尚未投票的提案。生产与测试环境始终分开标记；达到审批阈值并通过实时风控后会自动执行。':'This inbox combines current-team proposals requiring your independent decision. Production and Testnet remain distinct; execution starts automatically after the approval threshold and live-risk checks pass.',
  ...
  '这里展示草稿、等待审核和已批准但仍需跟踪的提案；批准后仍须完成实时风险检查与短期授权。':'Drafts, pending reviews, and approved proposals that still require follow-up appear here. Approval still requires live risk checks and short-lived authorization.', '这里展示草稿、等待审核，以及仍在启动窗口内的已批准提案；审核达标后系统自动完成实时风控、授权和执行。':'Drafts, pending reviews, and approved proposals still inside the launch window appear here. After approval, the system automatically completes live risk, authorization, and execution.',
  '这里只展示仍在草稿或等待审核中的提案；批准后进入历史，后续交易生命周期由风险管理接手。':'Only drafts and pending reviews appear here. Approved proposals move to History and are handed to risk management.', '这里只展示仍在草稿或等待审核中的提案；批准后进入历史，后续交易生命周期由系统自动推进。':'Only drafts and pending reviews appear here. Approved proposals move to History and the system advances their trade lifecycle automatically.',
  ...
  '二选一':'Choose one', '仅 Arbitrum':'Arbitrum only', '链上金库':'On-chain treasury', '实时额度预检':'Live limit preflight',
  ...
  'Hyperliquid 充值桥地址（Bridge）':'Hyperliquid deposit bridge address (Bridge)',
  '资金网络与结算资产':'Network and settlement asset',
  '资金网络':'Network',
  'Arbitrum One（Chain ID 42161）':'Arbitrum One (Chain ID 42161)',
  '结算资产':'Settlement asset',
  '原生 USDC':'Native USDC',
  '支持范围':'Supported scope',
  '当前默认仅支持 Arbitrum One 原生 USDC，不支持 USDT、其他网络资产或跨链版本 USDC。':'Only native USDC on Arbitrum One is supported by default. USDT, assets on other networks, and bridged variants of USDC are not supported.',
  '用于 Arbitrum One 原生 USDC 充值和接收资金的钱包地址。钱包需准备少量 ETH 支付链上 Gas。':'Wallet address used to send and receive native USDC on Arbitrum One. Keep a small amount of ETH available for network gas.',
  '仅用于 Arbitrum One 原生 USDC。生产环境必须使用 Hyperliquid 官方 Bridge2 地址。':'For native USDC on Arbitrum One only. Production must use the official Hyperliquid Bridge2 address.',
  '单笔资金划转允许的最高 USDC 手续费。当前费用超过此上限时，系统将阻止划转；该费用由交易所或资金通道收取，不支付给 TradeOps。':'The maximum USDC fee allowed for one capital transfer. The transfer is blocked when the current fee exceeds this limit. The fee is charged by the exchange or capital rail, not TradeOps.',
  '最大费用上限必须低于单次金额上限。':'The maximum fee must be lower than the per-operation limit.',
  ...
  '检查币安回流条件':'Check Binance return requirements', '检查 Hyperliquid 回流条件':'Check Hyperliquid return requirements', '检查 Hyperliquid CCTP 回流条件':'Check Hyperliquid CCTP return requirements',
  ...
  '当前运行中的交易任务没有阻断异常':'No blocking exceptions in active trades', '成员权限':'Access control', '权限分离原则':'Separation of duties', '审核、风险、资金与身份保持独立授权':'Keep review, risk, capital, and identity independently authorized',
  ...
  '等待首个快照':'Waiting for first snapshot', '按需读取':'On-demand read', '无数据':'No data',
  '旧快照、失败或降级状态不会标记为实时。':'Stale, failed, or degraded snapshots are never marked as live.',
  ...
  '当前唯一推荐动作':'Recommended next action', '交易任务已完成并关闭':'Trade completed and closed',
  '风险预留已释放，成交与对账记录保留在当前交易任务中。':'Risk reservations are released. Fill and reconciliation records remain attached to this trade.', '风险预留已释放，成交与对账记录保留在当前交易历史中。':'Risk reservations are released. Fill and reconciliation records remain in trade history.',
  '当前仓位':'Current position', '原生保护':'Native protection', '当前无仓位':'No open position',
  '当前无仓位，无需保护':'No open position; protection is not required', '进行中操作':'Active action',
  '无进行中意图':'No active intent', '不会与新动作冲突':'No active intent conflicts with the next action',
  '最近对账':'Latest reconciliation', '晚于当前仓位与操作记录':'Newer than the current position and action records',
  '已实现盈亏':'Realized PnL', '未实现盈亏':'Unrealized PnL', '风险目标':'Risk target',
  '执行记录':'Execution history', '订单操作与成交记录':'Order actions and fills', '尚无订单操作。':'No order actions yet.',
  '仓位数据':'Position data', '仓位数量':'Position quantity', '平均入场':'Average entry', '仓位更新时间':'Position updated',
  '保护状态':'Protection status', '保护数量':'Protection quantity', '保护触发价':'Protection trigger', '保护更新时间':'Protection updated',
  '最终仓位':'Final position', '目标数量':'Target quantity', '紧迫度':'Urgency', '目标原因':'Target reason',
  '对账':'Reconciliation', '对账结论':'Reconciliation result', '订单、成交、仓位和风险保护当前一致。':'Orders, fills, positions, and protection are reconciled.',
  '尚未运行对账；任何不确定结果都必须先对账。':'Reconciliation has not run. Reconcile every uncertain result before continuing.',
  '无风险预留。':'No risk reservation.', '已成交':'Filled', '已释放':'Released', '立即':'Immediate',
  '初仓':'Initial entry', '加仓':'Scale in', '减仓':'Reduce', '退出':'Exit',
  '会增加风险':'Adds risk', '只减仓':'Reduce only', '受控执行恢复：交易所成交与仓位已经核对，目标已降至 0。':'Controlled execution recovery: exchange fills and positions were reconciled, and the target was reduced to zero.',
  '当前没有必须处理的异常。继续观察；需要时可使用下方减仓或退出，加仓仍需通过全部门控。':'No required action is pending. Continue monitoring; reduce or exit when needed. Any scale-in must still pass every gate.',
  '仓位已确认且保护完整':'Position confirmed and fully protected', '当前无需动作':'No action required',
  '等待交易所仓位数据':'Waiting for exchange position data', '需要在最新数据后重跑':'Rerun after the latest data',
  '有仓位时必须确认足额覆盖':'An open position requires confirmed full protection',
  '风险预留与唯一目标':'Risk reservation and single target', '尚未设置':'Not set',
  '提案通过审核和风险检查后，风险管理人员才能发起开仓。':'Risk management can open a position only after proposal approval and risk checks pass.',
  ...
  '交易管理可用，但突破榜单机会源受限':'Trading management is available, but the breakout feed is limited',
  ...
  '风险管理人员看不到资金中心；系统管理员拥有最高管理权限，但资金动作仍受实时校验、最终确认和 Gate 约束。':'Risk managers cannot access Capital. System administrators have the highest administrative permission, while capital actions still require live validation, final confirmation, and safety gates.',
  ...
  '资金管理':'Treasury management', '查看与管理资金数据、划转和资金对账；与风险管理职责分离。':'View and manage capital data, transfers, and capital reconciliation separately from risk management.',
  ...
  'Perptape 机会源':'Perptape opportunity feed', '接入状态':'Connection status',
  ...
  'WebSocket 当前不可用；页面继续展示团队轮询快照，并明确保留采集时间与时效阻断。':'WebSocket is currently unavailable. The page continues to show the team polling snapshot, with collection time and freshness blockers preserved.',
  ...
  '风险管理或团队管理员':'Risk manager or team administrator', '未知服务端阻断':'Unknown server blocker',
  '提案、独立审核与风险管理职责已在至少一个精确账户范围内形成闭环。':'Proposal creation, independent review, and risk management form a complete control loop for at least one exact account scope.',
  ...
  '至少配置一名风险管理人员':'Assign at least one risk manager',
  ...
  '当前模式':'Current mode', '待配置':'Setup required', '生产模式':'Production mode', '切换当前模式':'Switch current mode', '关闭模式切换':'Close mode switch',
  '在页眉切换团队的测试与生产环境；每次切换仍由服务端检查权限、风险政策和当前执行事实。':'Switch the Team between Testnet and Production from the header. Every switch still requires server-side permission, risk-policy, and current-execution checks.',
  '正在读取当前模式…':'Loading current mode…', '正在核对切换权限、风险政策和当前执行事实。':'Checking switch permission, risk policy, and current execution facts.', '当前模式状态读取失败':'Current mode status could not be loaded',
  ...
  '账户配置范围':'Account configuration scope', '当前范围':'Current scope', '这里切换的只是账户配置范围，不会改变团队当前运行模式。实际模式切换请使用页眉“当前模式”。':'This switches only the account configuration scope and does not change the Team current mode. Use Current mode in the header to switch the actual mode.',
  '测试账户':'Testnet accounts', '生产账户':'Production accounts', '交易所测试环境 API':'Exchange Testnet API', '真实资金环境 API':'Real-funds API',
  ...
  '查看当前模式':'View current mode', '测试环境资金':'Testnet capital',
  '请由管理员使用页眉“当前模式”选择测试模式或生产模式。':'Ask an administrator to choose Testnet or Production from Current mode in the header.',
  ...
  '所选账户净值':'Selected-account net worth', '可信':'Trusted', '当前不可汇总':'Current total unavailable', '所选账户数据可信，可用于当前汇总':'Selected-account data is trusted and can be totaled', '当前汇总已阻断':'Current total is blocked', '需关注':'Attention required',
  ...
  '所选账户独立曲线与可信汇总':'Independent selected-account series and trusted total', '缺失、过期、错位和断档不会补零或强行连线。':'Missing, stale, misaligned, and gapped data is never zero-filled or forcibly connected.', '数据来自当前模式对应交易所只读事实':'Data comes from read-only exchange facts for the current mode.', '全屏查看':'View fullscreen', '退出全屏':'Exit fullscreen', '放大资金曲线':'Enlarge capital series', '退出资金曲线全屏':'Exit capital-series fullscreen',
  'WebSocket 健康检查已过期':'WebSocket health check is stale', 'WebSocket 当前不可用':'WebSocket is currently unavailable', 'HTTPS 轮询失败':'HTTPS polling failed',
  ...
  '交易历史':'Trade history', '交易历史记录':'Trade-history records', '每条交易历史保留从授权、风险占用和下单意图，到成交、保护、减仓、对账与最终结果的完整生命周期。':'Each trade-history record preserves the full lifecycle from authorization, risk reservation, and order intent through fills, protection, reductions, reconciliation, and final outcome.', '筛选交易历史':'Filter trade history', '按标的、方向、交易所和状态缩小结果':'Narrow results by instrument, direction, venue, and status', '搜索标的或账户':'Search instrument or account', '全部交易所':'All venues', '交易历史分页':'Trade-history pagination', '没有符合条件的交易历史':'No trade history matches these filters', '提案达到独立审核阈值后，系统会自动完成实时风控、短期授权、风险预留和受控执行。':'After a proposal reaches the independent-review threshold, the system automatically completes live risk checks, short-lived authorization, risk reservation, and controlled execution.', '返回交易历史':'Back to trade history', '查看交易历史':'View trade history', '该交易记录不存在':'This trade record does not exist', '该交易记录不属于当前控制台':'This trade record is outside the current console', '这里只展示交易所测试环境或生产环境的交易历史。':'Only exchange Testnet or Production trade history appears here.', '交易记录':'Trade record', '已结束任务不会占用当前风险工作区；可前往交易历史查看完整记录。':'Completed tasks do not occupy the current risk workspace; open Trade history for the complete record.',
  '交易任务记录':'Trade records', '建仓中 / 持仓中':'Opening / Open', '运行范围':'Runtime scope', '提案通过独立审核、实时风险检查和短期授权后，风险管理人员才能在同一环境发起订单。':'Only after independent review, real-time risk checks, and short-lived authorization may a risk manager submit an order in the same environment.',
  ...
  'OKX · 暂不支持测试环境执行':'OKX · Testnet execution is not supported', 'Bybit · 暂不支持测试环境执行':'Bybit · Testnet execution is not supported', 'OKX · 仅事实同步':'OKX · Facts only', 'Bybit · 仅事实同步':'Bybit · Facts only', '该交易所测试环境仅支持事实同步；执行保持不可用':'This venue supports testnet facts only; execution remains unavailable', '测试凭据只会加载到 TESTNET Adapter；生产凭据只会加载到 LIVE Adapter。':'Testnet credentials load only into the TESTNET Adapter; Production credentials load only into the LIVE Adapter.', '添加账户':'Add account',
  // Capital center.
  ...
  '政策正常':'Policy normal', '生产范围':'Production scope', '个范围待处理':'scopes need attention', '个范围受阻':'scopes blocked', '实时条件':'Live requirements', '日常团队风险政策':'Routine Team risk policy', '管理员直接修改':'Administrator edit', '管理员可直接修改所有阈值。保存会创建新版本，并使本团队旧交易授权全部失效。':'Administrators may edit all thresholds directly. Saving creates a new version and invalidates existing Team trading authorizations.',
  '数据完整性保护':'Data integrity protection', '所选区间缺少边界净值':'Boundary net worth is missing', '起始日或截止日没有可信净值点，因此本次报表没有生成。':'The start or end date has no trusted net-worth point, so the report was not generated.', '调整时间范围，确保开始和结束日期都有账户净值记录。':'Adjust the time range so both boundary dates have account net-worth records.',
  '净值历史尚不足':'Net-worth history is insufficient', '当前可信净值记录不足以形成连续收益率，因此本次报表没有生成。':'Trusted net-worth records are insufficient to produce a continuous return series, so the report was not generated.', '缩短时间范围，或等待账户净值同步覆盖所选日期。':'Shorten the time range or wait for account net-worth sync to cover the selected dates.',
  '所选账户范围不可用':'The selected account scope is unavailable', '当前身份、环境或交易所范围内没有可用于生成报表的账户事实。':'No account facts are available for reporting within the current identity, environment, or exchange scope.', '重新选择有权限的账户与环境后再检查。':'Select an authorized account and environment, then check again.',
  '估值事实尚不完整':'Valuation facts are incomplete', '汇率、仓位、现金流或基准事实存在缺口，因此系统拒绝生成可能误导的收益率。':'Exchange-rate, position, cash-flow, or benchmark facts are missing, so the system rejected a potentially misleading return series.', '等待缺失事实同步完成，或选择数据完整的账户和时间范围。':'Wait for missing facts to sync, or choose an account and time range with complete data.',
  '报表范围已经变化':'The report scope has changed', '当前权限或团队范围与请求发起时不一致，因此本次报表没有生成。':'The current permissions or Team scope no longer match the request, so the report was not generated.', '刷新页面并按当前可见范围重新选择条件。':'Refresh the page and choose filters from the currently visible scope.',
  '报表暂不可生成':'The report is not available yet', '服务端没有确认本次报表所需的全部可信事实。':'The server has not confirmed all trusted facts required for this report.', '调整筛选条件后重新检查；问题持续存在时再查看技术详情。':'Adjust the filters and check again; open technical details only if the issue persists.',
  '未生成':'Not generated', '下一步':'Next step', '调整时间范围':'Adjust time range', '重新检查':'Check again', '查看技术详情':'View technical details',
  '按当前团队已确认的风险授权边界配置版本化政策':'Configure a versioned policy from the Team approved risk boundary', '直接保存新版本':'Save new version', '当前团队控制状态':'Current Team control state', '团队最大总风险':'Maximum Team risk', '单账户最大风险':'Maximum account risk', '最大单笔亏损':'Maximum single-trade loss', '连续亏损 / 冷却':'Consecutive losses / cooldown', '最大持仓名义价值':'Maximum position notional', '加仓间距（bps）':'Add spacing (bps)', '布林中轨参考周期':'Bollinger midline reference periods', '低风险最多加仓':'Low-risk maximum adds', '中风险最多加仓':'Medium-risk maximum adds', '高风险最多加仓':'High-risk maximum adds', '低风险最大亏损比例':'Low-risk maximum loss fraction', '中风险最大亏损比例':'Medium-risk maximum loss fraction', '高风险最大亏损比例':'High-risk maximum loss fraction', '最大持仓':'Maximum position', '加仓间距 / 中轨周期':'Add spacing / midline periods', '风险档位加仓次数':'Risk-tier add counts', '风险档位最大亏损':'Risk-tier maximum loss',
  ...
  '动态范围 / 当前任务':'Dynamic scope / current task', '全部有效永续合约':'All active perpetual contracts', '当前没有待执行任务交易对':'No pending trade pair', '查看完整合约目录':'View full contract catalog', '搜索合约':'Search contracts', '允许':'Allowed', '动态合约范围':'dynamic contract scope',
  ...
  '生产确认文案已复制':'Production confirmation text copied', '浏览器未允许复制，请手动输入确认文案':'The browser did not allow copying. Enter the confirmation text manually.', '确认切换':'Confirm switch',
  '生产模式订单会影响真实资金；危险能力仍保持独立关闭。':'Production-mode orders can affect real funds. Dangerous capabilities remain independently disabled.', '测试模式订单会发送到交易所测试服务器。':'Testnet-mode orders are sent to exchange Testnet servers.',
  ...
  '系统状态仍展示风险政策、外部连接、执行底座和通知健康；运行任务、保护与对账详情由风险管理人员查看。':'System status still shows risk policy, external connections, execution infrastructure, and notification health. Active-trade, protection, and reconciliation details are available to risk managers.',
  ...
  '由风险管理人员查看':'Handled by a risk manager', '由机会创建者查看':'Handled by an opportunity creator',
  ...
  '返回交易任务':'Back to trades', '运行问题':'Runtime issues',
  ...
  '仅最高管理员可以直接恢复':'Only a super administrator can restore directly', '当前身份没有风险管理权限':'Your role does not have risk management access', '当前身份不是独立审核人员':'Your role is not an independent reviewer', '由资金管理员处理':'Handled by a treasury administrator', '当前身份可以查看服务端事实，但没有信号源管理权限。':'You can view server facts but cannot manage signal sources.',
  ...
  '例如 BTC、XYZ100':'For example BTC or XYZ100', '例如 BTCUSDT':'For example BTCUSDT', '例如 my-trading-bot':'For example my-trading-bot', '有效时间（小时，至少 8 小时）':'Validity (hours, at least 8)',
  ...
  '绑定范围':'Bound scope', '端点':'Endpoint', '未配置或无权查看':'Not configured or not visible', '错误代码':'Error code', '真实订单发送':'Live order sending', '本页不提供；服务端 Gate 独立控制':'Not available on this page; the service-side gate remains independent', '验证 Worker':'Verify worker', '先保存当前账户专属 Worker，再运行无下单验证。':'Save the exact-account worker before running the no-order verification.', '自动绑定当前账户':'Automatically bound to the current account', '空间和账户范围由当前页面自动确定，无需填写 URL。':'The current page determines the workspace and account scope; no URL is required.', '执行模式':'Execution mode', 'Worker 名称':'Worker name', 'Worker 服务地址':'Worker endpoint', '已按部署配置自动选择 · 不是工作空间或账户 URL':'Selected from deployment configuration · not a workspace or account URL', '控制用户名':'Control username', '控制密码':'Control password', 'HIP-3 DEX 白名单':'HIP-3 DEX allowlist', '保存 Worker 新版本':'Save new worker version', '启用账户交易资格':'Enable account trading eligibility', '先完成只读连接验证。':'Complete read-only connection verification first.',
  '全部有效 Core 与账户全部 HIP-3 永续合约':'All active Core and all account HIP-3 perpetual contracts', '系统按官方目录自动发现；下架、暂停或执行参数不完整的合约继续阻断。':'The system discovers them from the official catalog; delisted, paused, or execution-incomplete contracts remain blocked.',
  '账户地址':'Account address', 'API Wallet 地址（可选）':'API Wallet address (optional)', 'API Wallet 私钥（配置交易身份时必填）':'API Wallet private key (required for trading identity)', '加密保存且不会回显':'Encrypted and never displayed', '金库编号':'Vault ID', '金库地址':'Vault address', '币安默认账户':'Default Binance account', '币安账户 ID':'Binance account ID', 'Hyperliquid 账户 ID':'Hyperliquid account ID', '币安白名单入金地址':'Binance allowlisted deposit address', '币安受限提现地址':'Binance restricted withdrawal address', '默认账户':'Default account', '单次金额上限（USDC）':'Per-operation limit (USDC)', '最大费用上限（USDC）':'Maximum fee (USDC)', '必须低于单次金额上限':'Must be lower than the per-operation limit',
  '直接填写与服务端生产运行绑定完全一致的 account_id；账户名称不参与校验。':'Enter the account_id that exactly matches the Production runtime binding; the account name is not validated.', '两种链上金库共用币安、Hyperliquid、自有地址和金额限制；直接填写与服务端生产运行绑定一致的精确 account_id，不填写可编辑的账户名称。':'Both on-chain vaults share Binance, Hyperliquid, owned addresses, and limits. Enter the exact account_id bound to the Production runtime, not the editable account name.', '资金中心填写账户时使用上方精确账户 ID，不使用可编辑的账户名称。':'Use the exact account ID above in Capital Center, not the editable account name.', '账户 ID 必须与服务端运行绑定一致；账户名称仅用于页面识别，可在创建后修改。':'The account ID must match the server runtime binding. The account name is only for display and can be changed later.',
  '清除未使用的 NoTilt 配置':'Clear the unused NoTilt configuration', '清除未使用的 Safe 配置':'Clear the unused Safe configuration', '仅交易所或 Hyperliquid 提现会从 USDC 到账额扣除费用；Safe 发起的链上转账使用 ETH 支付 Gas，不再误减 USDC 到账额。':'Only exchange or Hyperliquid withdrawals deduct fees from received USDC. Safe-originated transfers pay gas in ETH and no longer reduce the USDC amount.', '实时可读':'Live read verified', 'Safe 余额':'Safe balance', '重置周期 / nonce':'Reset period / nonce', '尚未读取':'Not read yet', 'Hyperliquid 入金额度不足':'Insufficient Hyperliquid deposit allowance', '请先在 Safe Allowance Module 提升 delegate 可用额度。':'Increase the delegate allowance in the Safe Allowance Module first.',
  '必须与当前 Safe Smart Account 完全一致':'Must exactly match the current Safe Smart Account', '必须与当前 NoTilt 金库地址完全一致':'Must exactly match the current NoTilt vault address',
  '最近一次资金操作':'Latest capital operation', '资金路径已完成，公开回执已确认':'Capital route completed and public receipts confirmed', 'Safe 已到账，正在完成最终状态同步':'Safe credited; final status is syncing', 'Hyperliquid 提现已确认，正在等待 Arbitrum / Safe 到账':'Hyperliquid withdrawal confirmed; waiting for Arbitrum / Safe credit', '钱包提交成功，正在核对 Hyperliquid 账本':'Wallet submission succeeded; verifying the Hyperliquid ledger', '资金操作已提交，正在核对公开回执':'Capital operation submitted; verifying public receipts', '安全预检尚未通过，未提交资金操作':'Safety preflight has not passed; no capital operation was submitted', '资金操作已创建，正在准备下一步':'Capital operation created; preparing the next step', '查看 Arbitrum 回执':'View Arbitrum receipt',
  '优先使用 Hyperliquid 当前官方 CCTP 路由，固定扣除 0.2 USDC 协议费；仅在官方路由明确回退时使用 Legacy Bridge。':'Use Hyperliquid’s current official CCTP route with a fixed 0.2 USDC protocol fee; use the Legacy Bridge only when the official route explicitly falls back.', '只允许提现到冻结的 Arbitrum 金库地址，固定扣除 0.2 USDC 协议费；旧 Legacy Bridge 路由会由服务端拒绝。':'Withdrawals are allowed only to the frozen Arbitrum treasury address with a fixed 0.2 USDC protocol fee; the server rejects the legacy Bridge route.', 'CCTP 提现':'CCTP withdrawal', '官方提现':'Official withdrawal', 'Arbitrum 确认':'Arbitrum confirmation', '金库到账':'Treasury credit',
  '最大费用上限必须低于单次金额上限，请调整后重新保存。':'The maximum fee must be lower than the per-operation limit. Adjust it and save again.',
  '币安受限提现地址必须与当前 Safe Smart Account 完全一致，请核对后重新保存。':'The Binance restricted withdrawal address must exactly match the current Safe Smart Account. Verify it and save again.',
  '币安受限提现地址必须与当前 NoTilt 金库地址完全一致，请核对后重新保存。':'The Binance restricted withdrawal address must exactly match the current NoTilt vault address. Verify it and save again.',
  '资金路径账户必须与当前生产运行绑定的默认账户一致，请在账户管理中核对后重试。':'Capital-path accounts must match the default accounts bound to the current production runtime. Verify them in Account management and try again.',
  '请填写与服务端生产运行绑定完全一致的账户 ID。系统校验不可变 account_id，不使用可编辑的账户名称。':'Enter the account ID that exactly matches the Production runtime binding. Validation uses the immutable account_id, not the editable account name.',
  '币安受限提现地址必须与当前使用的链上金库地址完全一致。使用 Safe 时请填写 Safe Smart Account；使用 NoTilt 时请填写 NoTilt 金库地址。':'The Binance restricted withdrawal address must exactly match the selected on-chain treasury address. Use the Safe Smart Account for Safe or the NoTilt vault address for NoTilt.',
  'NoTilt 金库地址与当前可信生产范围不一致，请核对金库地址后重试。':'The NoTilt vault address does not match the trusted production scope. Verify the vault address and try again.',
...
const ENGLISH_PATTERNS = [
  [/^当前没有(.+)交易历史$/, (_match, mode) => `There is no ${translateEnglishText(mode)} trade history`],
  [/^全部有效永续合约 · (\d+) 个$/, 'All active perpetual contracts · $1'],
  [/^当前任务：(.+) · (允许|阻断)$/, (_match, pair, status) => `Current task: ${pair} · ${status === '允许' ? 'Allowed' : 'Blocked'}`],
  ...
  [/^连接验证失败：(.+)$/, 'Connection verification failed: $1'],
  ...
  [/^(.+) · (.+) · 精确空间\/账户范围$/, '$1 · $2 · Exact workspace/account scope'],
  ...
  [/^(\d+) \u4e2a\u8303\u56f4\u53d7\u963b$/, '$1 scopes blocked'],
  [/^(\d+) \u6b21 \/ (\d+) \u79d2$/, '$1 times / $2 sec'],
  [/^\u653f\u7b56\u6b63\u5e38\uff0c\u4f46\u751f\u4ea7\u4e8b\u5b9e\u4ecd\u6709\u963b\u585e\u6761\u4ef6\uff1a$/, 'The policy is normal, but Production facts still contain blockers:'],
  [/^(\d+) \u6761\u542f\u7528$/, '$1 enabled'],
  [/^(\d+) \u6761$/, '$1 routes'],
  [/^交易 (.+) · 资金 (.+) · 签名 (.+) · 广播 (.+)。测试通知只入队；独立通知进程发送文本，不触发业务动作。$/, 'Trading $1 · Capital $2 · Signing $3 · Broadcast $4. Test notifications only enter the queue; the independent worker sends text and cannot trigger business actions.'],
  ...
  [/^确认切换到(测试模式|生产模式)？$/, (_match, mode) => `Switch to ${mode === '测试模式' ? 'Testnet mode' : 'Production mode'}?`],
  [/^已切换到(测试模式|生产模式)$/, (_match, mode) => `Switched to ${mode === '测试模式' ? 'Testnet mode' : 'Production mode'}`],
  ...
  [/^剩余 (值|\d+) 小时 (值|\d+) 分钟$/, (_match, hours, minutes) => `${hours} ${hours === '1' ? 'hour' : 'hours'} ${minutes} ${minutes === '1' ? 'minute' : 'minutes'} remaining`],
  [/^(\d+) 小时 (\d+) 分钟$/, (_match, hours, minutes) => `${hours} ${hours === '1' ? 'hour' : 'hours'} ${minutes} ${minutes === '1' ? 'minute' : 'minutes'}`],
  [/^(.+) · 剩余 (\d+) 小时 (\d+) 分钟$/, (_match, prefix, hours, minutes) => `${prefix} · ${hours} ${hours === '1' ? 'hour' : 'hours'} ${minutes} ${minutes === '1' ? 'minute' : 'minutes'} remaining`],
  [/^(.+) · 剩余 (\d+) 分钟$/, (_match, prefix, minutes) => `${prefix} · ${minutes} ${minutes === '1' ? 'minute' : 'minutes'} remaining`],
  [/^(.+) · 剩余 (\d+) 小时$/, (_match, prefix, hours) => `${prefix} · ${hours} ${hours === '1' ? 'hour' : 'hours'} remaining`],
  [/^初仓 (.+)$/, 'Initial entry $1'],
  [/^距触发 (.+)$/, '$1 from trigger'],
  [/^核对名义价值、最大风险、失效位置和创建时来源快照。高风险提案需要两名不同审核人；当前已记录 (\d+) 票。剩余 (\d+) 小时 (\d+) 分钟。$/, (_match, votes, hours, minutes) => `Check the notional value, maximum risk, invalidation level, and creation-time source snapshot. High-risk proposals require two different reviewers; ${votes} votes are recorded. ${hours} ${hours === '1' ? 'hour' : 'hours'} ${minutes} ${minutes === '1' ? 'minute' : 'minutes'} remain.`],
  [/^核对名义价值、最大风险、失效位置和创建时来源快照。高风险提案需要两名不同审核人；当前已记录 (\d+) 票。达到所需票数并通过实时风控后，系统会自动授权、预留并交给 Freqtrade 执行。(.+)。$/, (_match, votes, remaining) => `Check the notional value, maximum risk, invalidation level, and creation-time source snapshot. High-risk proposals require two different reviewers; ${votes} votes are recorded. After the threshold and live-risk checks pass, the system automatically authorizes, reserves, and hands execution to Freqtrade. ${translateEnglishText(remaining)}.`],
  [/^核对名义价值、最大风险、失效位置和创建时来源快照。批准并通过实时风控后，系统会自动授权、预留并交给 Freqtrade 执行。(.+)。$/, (_match, remaining) => `Check the notional value, maximum risk, invalidation level, and creation-time source snapshot. After approval and live-risk checks pass, the system automatically authorizes, reserves, and hands execution to Freqtrade. ${translateEnglishText(remaining)}.`],
  [/^剩余 (.+)$/, '$1 remaining'],
  ...
  [/^创建提案时，突破榜单中同一精确合约、同一方向在 (.+) 同时突破。(.*)来源与参数已冻结；达到人工审核阈值并通过实时风控后，系统自动推进受控交易。$/, (_match, timeframes, rationale) => `At creation, the same exact contract and direction broke out across ${timeframes}. ${translateEnglishText(rationale)}The source and parameters are frozen. After the human-review threshold and live-risk checks pass, the controlled trade advances automatically.`],
  [/^系统将记录你的一次独立批准。达到所需审批票数并通过实时风控后，会按已冻结的账户 (.+) \/ 场所 (.+) \/ 标的 (.+) \/ 方向 (.+)（(BUY|SELL)）\/ 类型 (.+) \/ 数量 (.+) \/ 预计名义价值 (.+) \/ 杠杆 (.+)x \/ 最大风险 (.+) 自动签发授权、预留风险并由 Freqtrade 发送(测试环境订单|生产真实订单)、查询成交和对账；后续不再要求人工点击。超时或状态不明时只查询，不重复下单。$/, (_match, account, venue, symbol, direction, side, orderType, quantity, notional, leverage, risk, orderKind) => `The system records one independent approval. After the approval threshold and live-risk checks pass, it automatically authorizes, reserves risk, and asks Freqtrade to send the frozen account ${account} / venue ${venue} / symbol ${symbol} / direction ${translateEnglishText(direction)} (${side}) / type ${orderType} / quantity ${quantity} / estimated notional ${notional} / leverage ${leverage}x / maximum risk ${risk} ${orderKind === '生产真实订单' ? 'production live order' : 'Testnet order'}, then queries fills and reconciles. No further manual click is required. A timeout or unknown state is query-only and is never submitted again.`],
  [/^系统将记录你的一次独立批准。达到所需审批票数并通过实时风控后，会按已冻结的 (.+) \/ (.+) \/ 数量 (.+) \/ 最大风险 (.+) 自动签发授权、预留风险并由 Freqtrade 发送(测试环境订单|生产真实订单)；后续不再要求人工点击。$/, (_match, venue, direction, quantity, risk, orderKind) => `The system records one independent approval. After the approval threshold and live-risk checks pass, it automatically authorizes, reserves risk, and asks Freqtrade to send the frozen ${venue} / ${translateEnglishText(direction)} / quantity ${quantity} / maximum risk ${risk} ${orderKind === '生产真实订单' ? 'production live order' : 'Testnet order'}. No further manual click is required.`],
  ...
  [/^当前没有运行阻断。生产 (\d+) 笔，测试 (\d+) 笔；批准前核对冻结参数，审核达标并通过实时风控后系统会自动执行。$/, 'There is no runtime blocker. Production: $1; Testnet: $2. Verify frozen parameters before approval; the system executes automatically after the threshold and live-risk checks pass.'],
  ...
  [/^确认于 (.+)$/, 'Confirmed $1'],
  [/^对账一致 · (.+)$/, 'Reconciled · $1'],
  [/^(\d+) 个操作 · (\d+) 笔成交$/, '$1 actions · $2 fills'],
  [/^([0-9a-f]+…) · 会增加风险 · (.+)$/i, '$1 · Adds risk · $2'],
  [/^([0-9a-f]+…) · 只减仓 · (.+)$/i, '$1 · Reduce only · $2'],
  [/^(.+ · (?:LIVE|TESTNET))订单 (.+) · 已成交 (.+) \/ (.+)$/, '$1 order $2 · Filled $3 / $4'],
  [/^—（当前无仓位）$/, '— (no open position)'],
  [/^(\d+) 个交易任务$/, '$1 trades'], [/^(\d+) 项阻断$/, '$1 blockers'], [/^(\d+) 项需要处理$/, '$1 issues require action'],
  [/^(\d+) 项监控结果需要处理$/, '$1 monitoring results need attention'],
  [/^(\d+) 项阻断，查看详情$/, '$1 blockers; view details'],
  [/^(\d+) 项敞口不确定$/, '$1 exposure issues'], [/^(\d+) 项未一致$/, '$1 reconciliation issues'],
  [/^(\d+) 名启用成员$/, '$1 active users'], [/^(\d+) 个结果$/, '$1 results'], [/^(\d+) \/ (\d+) 个结果$/, '$1 / $2 results'], [/^显示更多（剩余 (\d+)）$/, 'Show more ($1 remaining)'], [/^(\d+) 条记录$/, '$1 records'],
  [/^(\d+) 个可连接$/, '$1 available'],
  ...
  [/^最近成功 (.+)$/, (_match, value) => `Latest success ${translateEnglishText(value)}`],
  [/^最近错误 (.+)$/, (_match, value) => `Latest error ${translateEnglishText(value)}`],
  [/^回执：(.+)$/, (_match, value) => `Receipt: ${translateEnglishText(value)}`],
