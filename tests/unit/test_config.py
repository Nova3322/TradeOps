...
def test_postgresql_psycopg_url_is_accepted() -> None:
    ...
    assert settings.binance_capital_base_url == "https://api.binance.com"
    assert settings.binance_capital_withdraw_enabled is False
    ...
    assert "binance_read_only_enabled" not in Settings.model_fields
    assert "hyperliquid_read_only_enabled" not in Settings.model_fields
    assert "binance_live_order_send_enabled" not in Settings.model_fields
    assert "hyperliquid_live_order_send_enabled" not in Settings.model_fields
...
def test_explicit_config_directory_loads_both_secret_and_production_layers(
    ...
    (tmp_path / ".env.local").write_text(
        "TRADING_DATABASE_URL=postgresql+psycopg://user:pass@localhost/trading\n"
        "TRADING_BINANCE_CAPITAL_API_KEY=fixture-key\n"
        "TRADING_BINANCE_CAPITAL_API_SECRET=fixture-secret\n"
    )
    (tmp_path / ".env.production.local").write_text(
        "TRADING_BINANCE_CAPITAL_ACCOUNT_ID=binance-main\n"
    )
    monkeypatch.setenv("TRADING_CONFIG_DIR", str(tmp_path))
    get_settings.cache_clear()
    try:
        settings = get_settings()
        assert settings.binance_capital_account_id == "binance-main"
        assert settings.binance_capital_api_key == "fixture-key"
...
def test_capital_credentials_are_dedicated_and_require_an_exact_account() -> None:
    database_url = "postgresql+psycopg://user:pass@localhost/trading"
    missing_secret = Settings(
        database_url=database_url,
        binance_capital_api_key="fixture-key",
        _env_file=None,
    )
    with pytest.raises(ValueError, match="key and secret"):
        missing_secret.validate_runtime_security()

    missing_account = Settings(
        database_url=database_url,
        binance_capital_api_key="fixture-key",
        binance_capital_api_secret="fixture-secret",  # noqa: S106
        _env_file=None,
    )
    with pytest.raises(ValueError, match="exact dedicated capital account ID"):
        missing_account.validate_runtime_security()
...
def test_fact_and_freqtrade_runtimes_require_database_bound_credentials() -> None:
    database_url = "postgresql+psycopg://user:pass@localhost/trading"
    missing_fact_dependencies = Settings(
        database_url=database_url,
        fact_adapter_enabled=True,
        _env_file=None,
    )
    with pytest.raises(ValueError, match="runtime sync, credential encryption"):
        missing_fact_dependencies.validate_runtime_security()

    missing_worker_key = Settings(
        database_url=database_url,
...
@pytest.mark.parametrize(
    ...
    [
        (
            {"binance_capital_api_key": "fixture-key"},
            "capital key and secret",
        ),
        (
            {
                "binance_capital_api_key": "fixture-key",
                "binance_capital_api_secret": "fixture-secret",
            },
            "exact dedicated capital account ID",
        ),
        (
            {
                "fact_adapter_enabled": True,
            },
            "runtime sync, credential encryption",
        ),
        (
            {
                "freqtrade_workers_enabled": True,
