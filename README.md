...
当前已完成三十二个本地工程工作包：从 [WP-0001：耐久控制面基础](docs/08-implementation/WP-0001-耐久控制面基础.md) 的命令幂等、不可变审计和 outbox/inbox，推进到 [WP-0032：Scope Stress 策略派生](docs/08-implementation/WP-0032-Scope-Stress策略派生.md)。WP-0012 至 WP-0017 将 canonical order/fill/position/protection 事实逐层强绑定到 OrderIntent、风险暴露与原生保护；WP-0018 至 WP-0020 建立账户权益、当前投影和不可变受管账户 manifest；WP-0021/0022 分别在提案与最终预检重算 canonical 组合资本，并保持冻结 `Snapshot_0` 与 1R 不变；WP-0023/0024 通过同一公共聚合器，在最终预检和提案预检从 Risk Ledger 当前态派生 funding、Heat、内部 margin 和七层 scope；WP-0025 冻结三项损失分量；WP-0026 让 base Heat 与 scope planned loss 共用服务端公式；WP-0027 移除 caller-reported cost，改从无生产默认的版本化 Risk Policy 派生 fee、stop penetration 和 adverse funding stress；WP-0028 至 WP-0030 建立最差活动保护触发价、current-position exact 投影及结算币种内互斥的 Open Heat/Giveback；WP-0031 再移除 caller giveback，并在 ADD 最终预检把 canonical 当前保护风险同步绑定到 Trade Loss、七层 scope 和决策有效期；WP-0032 删除 caller scope stress，改由每个 scope 的冻结版本化压力场景派生并写入 Reservation。技术选择见 [ADR-0001](docs/08-implementation/ADR-0001-生产技术栈与首包边界.md)，逐包验证证据位于 [docs/08-implementation/evidence](docs/08-implementation/evidence/)。
