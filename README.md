...
当前已完成九个本地工程工作包：从 [WP-0001：耐久控制面基础](docs/08-implementation/WP-0001-耐久控制面基础.md) 的命令幂等、不可变审计和 outbox/inbox，推进到 [WP-0009：耐久 ReconciliationRun 与最新成功门](docs/08-implementation/WP-0009-耐久ReconciliationRun与最新成功门.md) 的七源输入水位、不可变 Finding/Resolution、不可逆终态和 latest-success claim gate。WP-0008 已完成 exact-scope sender authority、单调 fencing token、短租约和不可发送 claim；WP-0007 已完成不可变证据包、精确 scope/版本/额度校验、证书单向状态和授权失效传播；WP-0006 已完成 final risk precheck、RiskReservation、守恒账本、Initial/Add claim 及部分/零/Unknown 对账。中间包覆盖服务端 IAM 判定、冻结提案审核、确定性风险预检和 TradingAuthorization/Campaign。技术选择见 [ADR-0001](docs/08-implementation/ADR-0001-生产技术栈与首包边界.md)，逐包验证证据位于 [docs/08-implementation/evidence](docs/08-implementation/evidence/)。

当前代码仍只有不可发送的 SHADOW OrderIntent、只允许 `real_funds_eligible=false` 的 SHADOW 证书控制合同，以及 `external_send_permitted=false` 的 SHADOW sender lease/claim。ReconciliationRun 只耐久化本地测试输入、水位与差异证据，不连接真实场所。没有 production/small-live 证书或 lease、worker/sidecar 发送瞬间 fencing、Freqtrade/VenueAdapter、VenueOrder/Fill、真实 position/protection 事实、Web/PWA、Telegram、Margin、Vault/CTO 或场所认证证据。迁移不 seed 证书、sender scope、lease、run 或 claim；`LIVE_ORDER_SEND`、`CAPITAL_TRANSFER`、`AUTO_ADD` 均为 `DISABLED`。工程节点不等于获得实盘授权。
