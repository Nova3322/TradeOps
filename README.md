...
当前已完成三十四个本地工程工作包：从 [WP-0001：耐久控制面基础](docs/08-implementation/WP-0001-耐久控制面基础.md) 的命令幂等、不可变审计和 outbox/inbox，推进到 [WP-0034：不可变原生保护能力强绑定](docs/08-implementation/WP-0034-不可变原生保护能力强绑定.md)。WP-0012 至 WP-0017 将 canonical order/fill/position/protection 事实逐层强绑定到 OrderIntent、风险暴露与原生保护；WP-0018 至 WP-0020 建立账户权益、当前投影和不可变受管账户 manifest；WP-0021/0022 分别在提案与最终预检重算 canonical 组合资本，并保持冻结 `Snapshot_0` 与 1R 不变；WP-0023/0024 通过同一公共聚合器，在最终预检和提案预检从 Risk Ledger 当前态派生 funding、Heat、内部 margin 和七层 scope；WP-0025 冻结三项损失分量；WP-0026 让 base Heat 与 scope planned loss 共用服务端公式；WP-0027 移除 caller-reported cost，改从无生产默认的版本化 Risk Policy 派生 fee、stop penetration 和 adverse funding stress；WP-0028 至 WP-0030 建立最差活动保护触发价、current-position exact 投影及结算币种内互斥的 Open Heat/Giveback；WP-0031 再移除 caller giveback，并在 ADD 最终预检把 canonical 当前保护风险同步绑定到 Trade Loss、七层 scope 和决策有效期；WP-0032 删除 caller scope stress，改由每个 scope 的冻结版本化压力场景派生并写入 Reservation；WP-0033 删除 caller classification boolean，以不可变 exact Catalog 记录同时强绑定提案与最终预检；WP-0034 删除 caller `protection_available`，以不可变 exact 原生保护能力记录绑定账户、凭证、worker、Catalog 和保护模板。技术选择见 [ADR-0001](docs/08-implementation/ADR-0001-生产技术栈与首包边界.md)，逐包验证证据位于 [docs/08-implementation/evidence](docs/08-implementation/evidence/)。

当前代码仍只有不可发送的 SHADOW OrderIntent、只允许 `real_funds_eligible=false` 的 SHADOW 证书、资金范围合同和原生保护能力记录，以及 `external_send_permitted=false` 的 SHADOW sender lease/claim。canonical VenueOrderObservation、VenueFill、VenuePositionSnapshot、VenueProtectionSnapshot 和 VenueAccountEquitySnapshot 只能由本地测试构造的完整对账输入产生；提案与最终 OrderIntent 预检已绑定组合资本、Ledger 当前占用、规范化 base Heat、策略绑定 cost、per-scope stress、Instrument Catalog 和 exact 原生保护能力，ADD 最终预检也已从 current position/protection 派生并强绑定 Open Heat/Giveback，同步重算七层 scope 当前 planned/stress 占用。尚未完成的是持续风险监控、退出仲裁和真实压力参数研究。正式 cost/scope-stress 参数、FX/USD、稳定币折扣、保护成交、滑点和资金费证据仍未认证。没有真实部署账户 manifest、真实私有 venue collector，也没有认证 Binance/Hyperliquid 的逐场所账户权益公式或 FX/stablecoin/depeg 事实。尚缺 production/small-live 证书或 lease、真实 Venue Capability 记录、worker/sidecar 发送瞬间 fencing、Freqtrade/VenueAdapter、跨币种 USD 组合 MTM、最终 PnL、Web/PWA、Telegram、Margin、Vault/CTO 或场所认证证据。迁移不 seed manifest、证书、sender scope、lease、run、claim、venue fact 或保护能力；`LIVE_ORDER_SEND`、`CAPITAL_TRANSFER`、`AUTO_ADD` 均为 `DISABLED`。工程节点不等于获得实盘授权。
