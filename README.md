...
当前已完成十二个本地工程工作包：从 [WP-0001：耐久控制面基础](docs/08-implementation/WP-0001-耐久控制面基础.md) 的命令幂等、不可变审计和 outbox/inbox，推进到 [WP-0012：Canonical VenueOrder 与 VenueFill 事实](docs/08-implementation/WP-0012-Canonical-VenueOrder与VenueFill事实.md) 的私有场所订单观察、全局成交去重、逐输入不可变 membership 和 exact item-count 门。WP-0010/0011 已关闭弱执行事实入口并允许 current successor lease 只收敛旧 claim；WP-0009 已完成七源输入水位、不可变 Finding/Resolution、不可逆终态和 latest-success claim gate；WP-0008 已完成 exact-scope sender authority、单调 fencing token、短租约和不可发送 claim。前序包还覆盖证书、原子风险预留、服务端 IAM、冻结提案审核、确定性风险预检和 TradingAuthorization/Campaign。技术选择见 [ADR-0001](docs/08-implementation/ADR-0001-生产技术栈与首包边界.md)，逐包验证证据位于 [docs/08-implementation/evidence](docs/08-implementation/evidence/)。

当前代码仍只有不可发送的 SHADOW OrderIntent、只允许 `real_funds_eligible=false` 的 SHADOW 证书控制合同，以及 `external_send_permitted=false` 的 SHADOW sender lease/claim。WP-0012 的 VenueOrderObservation/VenueFill 只接受本地测试构造的 private-venue 事实，并且不会修改 OrderIntent、Heat、Position 或 PnL；尚未连接真实场所。没有 production/small-live 证书或 lease、worker/sidecar 发送瞬间 fencing、Freqtrade/VenueAdapter、真实 collector、Position/Balance/Protection 权威事实、Web/PWA、Telegram、Margin、Vault/CTO 或场所认证证据。迁移不 seed 证书、sender scope、lease、run、claim 或 venue fact；`LIVE_ORDER_SEND`、`CAPITAL_TRANSFER`、`AUTO_ADD` 均为 `DISABLED`。工程节点不等于获得实盘授权。
