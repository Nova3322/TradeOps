...
当前已完成二十三个本地工程工作包：从 [WP-0001：耐久控制面基础](docs/08-implementation/WP-0001-耐久控制面基础.md) 的命令幂等、不可变审计和 outbox/inbox，推进到 [WP-0023：最终预检 Durable Exposure 强绑定](docs/08-implementation/WP-0023-最终预检Durable-Exposure强绑定.md)。WP-0012 至 WP-0017 将 canonical order/fill/position/protection 事实逐层强绑定到 OrderIntent、风险暴露与原生保护；WP-0018 至 WP-0020 建立账户权益、当前投影和不可变受管账户 manifest；WP-0021/0022 分别在提案与最终预检重算 canonical 组合资本，并保持冻结 `Snapshot_0` 与 1R 不变；WP-0023 在最终预检从 Risk Ledger 当前态精确派生 funding、Heat、内部 margin reservation 和七层 scope 使用量。技术选择见 [ADR-0001](docs/08-implementation/ADR-0001-生产技术栈与首包边界.md)，逐包验证证据位于 [docs/08-implementation/evidence](docs/08-implementation/evidence/)。

当前代码仍只有不可发送的 SHADOW OrderIntent、只允许 `real_funds_eligible=false` 的 SHADOW 证书与资金范围合同，以及 `external_send_permitted=false` 的 SHADOW sender lease/claim。canonical VenueOrderObservation、VenueFill、VenuePositionSnapshot、VenueProtectionSnapshot 和 VenueAccountEquitySnapshot 只能由本地测试构造的完整对账输入产生；最终 OrderIntent 预检已绑定组合资本与 Ledger 当前占用，但提案 RiskPrecheck 的 funding/Heat/scope 当前占用及动态 giveback/cost 组件仍未完成同类耐久绑定。没有真实部署账户 manifest、真实私有 venue collector，也没有认证 Binance/Hyperliquid 的逐场所账户权益公式或 FX/stablecoin/depeg 事实。尚缺 production/small-live 证书或 lease、worker/sidecar 发送瞬间 fencing、Freqtrade/VenueAdapter、跨币种 USD 组合 MTM、最终 PnL、Web/PWA、Telegram、Margin、Vault/CTO 或场所认证证据。迁移不 seed manifest、证书、sender scope、lease、run、claim 或 venue fact；`LIVE_ORDER_SEND`、`CAPITAL_TRANSFER`、`AUTO_ADD` 均为 `DISABLED`。工程节点不等于获得实盘授权。
