...
当前已完成二十一个本地工程工作包：从 [WP-0001：耐久控制面基础](docs/08-implementation/WP-0001-耐久控制面基础.md) 的命令幂等、不可变审计和 outbox/inbox，推进到 [WP-0021：RiskPrecheck 可信资本强绑定](docs/08-implementation/WP-0021-RiskPrecheck可信资本强绑定.md)。WP-0012/0013 建立 canonical order/fill 并把 exact fact、input link、claim client-order identity 和连续原生 order ID 强绑定到 OrderIntent 与风险暴露；WP-0014/0015 建立仓位事实并把完全匹配的 OPEN/FLAT 快照绑定到 `POSITION_RECONCILED`；WP-0016/0017 建立原生保护事实并只允许全覆盖、reduce-only、非 replacement 的 `CONFIRMED` 保护推进 `PROTECTION_CONFIRMED`；WP-0018/0019 建立精确账户权益事实和当前投影；WP-0020 用不可变 manifest 明确完整受管账户全集；WP-0021 让提案 RiskPrecheck 在事务内重算组合资本并强绑定 manifest、投影和 canonical source 证据，拒绝自报资本、陈旧事实和缺失 FX。技术选择见 [ADR-0001](docs/08-implementation/ADR-0001-生产技术栈与首包边界.md)，逐包验证证据位于 [docs/08-implementation/evidence](docs/08-implementation/evidence/)。

当前代码仍只有不可发送的 SHADOW OrderIntent、只允许 `real_funds_eligible=false` 的 SHADOW 证书与资金范围合同，以及 `external_send_permitted=false` 的 SHADOW sender lease/claim。canonical VenueOrderObservation、VenueFill、VenuePositionSnapshot、VenueProtectionSnapshot 和 VenueAccountEquitySnapshot 只能由本地测试构造的完整对账输入产生；只有提案阶段 RiskPrecheck 已绑定组合资本，最终 OrderIntent 预检及 funding/heat/scope exposure 仍未完成同类事实绑定。没有真实部署账户 manifest、真实私有 venue collector，也没有认证 Binance/Hyperliquid 的逐场所账户权益公式或 FX/stablecoin/depeg 事实。尚缺 production/small-live 证书或 lease、worker/sidecar 发送瞬间 fencing、Freqtrade/VenueAdapter、跨币种 USD 组合 MTM、最终 PnL、Web/PWA、Telegram、Margin、Vault/CTO 或场所认证证据。迁移不 seed manifest、证书、sender scope、lease、run、claim 或 venue fact；`LIVE_ORDER_SEND`、`CAPITAL_TRANSFER`、`AUTO_ADD` 均为 `DISABLED`。工程节点不等于获得实盘授权。
