...
当前已完成二十二个本地工程工作包：从 [WP-0001：耐久控制面基础](docs/08-implementation/WP-0001-耐久控制面基础.md) 的命令幂等、不可变审计和 outbox/inbox，推进到 [WP-0022：最终 ORDER_PRECHECK 可信资本强绑定](docs/08-implementation/WP-0022-最终ORDER-PRECHECK可信资本强绑定.md)。WP-0012 至 WP-0017 将 canonical order/fill/position/protection 事实逐层强绑定到 OrderIntent、风险暴露与原生保护；WP-0018 至 WP-0020 建立精确账户权益事实、可重建当前投影和不可变受管账户 manifest；WP-0021 在提案 RiskPrecheck 内重算 canonical 组合资本；WP-0022 把同一资金范围冻结进授权，并在最终增险预检刷新最新 MTM/UPNL/available margin，同时保持 `Snapshot_0` 与 1R 不变。技术选择见 [ADR-0001](docs/08-implementation/ADR-0001-生产技术栈与首包边界.md)，逐包验证证据位于 [docs/08-implementation/evidence](docs/08-implementation/evidence/)。

当前代码仍只有不可发送的 SHADOW OrderIntent、只允许 `real_funds_eligible=false` 的 SHADOW 证书与资金范围合同，以及 `external_send_permitted=false` 的 SHADOW sender lease/claim。canonical VenueOrderObservation、VenueFill、VenuePositionSnapshot、VenueProtectionSnapshot 和 VenueAccountEquitySnapshot 只能由本地测试构造的完整对账输入产生；提案与最终 OrderIntent 预检已绑定组合资本，但 funding/heat/scope exposure 仍是上游申报加 durable 不得低报检查，尚未完全由 Ledger 派生。没有真实部署账户 manifest、真实私有 venue collector，也没有认证 Binance/Hyperliquid 的逐场所账户权益公式或 FX/stablecoin/depeg 事实。尚缺 production/small-live 证书或 lease、worker/sidecar 发送瞬间 fencing、Freqtrade/VenueAdapter、跨币种 USD 组合 MTM、最终 PnL、Web/PWA、Telegram、Margin、Vault/CTO 或场所认证证据。迁移不 seed manifest、证书、sender scope、lease、run、claim 或 venue fact；`LIVE_ORDER_SEND`、`CAPITAL_TRANSFER`、`AUTO_ADD` 均为 `DISABLED`。工程节点不等于获得实盘授权。
