"""Add canonical private-venue account-equity snapshots.
...
down_revision: str | Sequence[str] | None = "20260718_0017"
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("venue_account_equity_snapshot_id", sa.Uuid(), nullable=False),
        ...
        sa.Column("equity_state", sa.String(length=20), nullable=False),
        sa.Column("wallet_balance", sa.Numeric(38, 18), nullable=True),
        sa.Column("exchange_margin_equity", sa.Numeric(38, 18), nullable=True),
        sa.Column("available_margin", sa.Numeric(38, 18), nullable=True),
        sa.Column("total_unrealized_pnl", sa.Numeric(38, 18), nullable=True),
        sa.Column("total_initial_margin", sa.Numeric(38, 18), nullable=True),
        sa.Column("total_maintenance_margin", sa.Numeric(38, 18), nullable=True),
        sa.Column("total_liability", sa.Numeric(38, 18), nullable=True),
        sa.Column("unsettled_fee", sa.Numeric(38, 18), nullable=True),
        sa.Column("unsettled_funding", sa.Numeric(38, 18), nullable=True),
        sa.Column("includes_unrealized_pnl", sa.Boolean(), nullable=False),
        ...
        sa.ForeignKeyConstraint(
            ...
            name="fk_venue_account_equity_snapshots_first_input",
            ondelete="RESTRICT",
        ),
        sa.ForeignKeyConstraint(
        ...
        sa.PrimaryKeyConstraint("venue_account_equity_snapshot_id"),
        sa.UniqueConstraint(
            "organization_id",
            "venue",
            "execution_domain",
    ...
    op.add_column(
        ...
        sa.Column("venue_account_equity_snapshot_id", sa.Uuid(), nullable=True),
    )
    op.create_foreign_key(
        "fk_venue_fact_input_links_account_equity_snapshot",
        "venue_fact_input_links",
        "venue_account_equity_snapshots",
    ...
    op.create_unique_constraint(
        "uq_venue_fact_input_links_account_equity_fact",
        "venue_fact_input_links",
        ["reconciliation_input_id", "venue_account_equity_snapshot_id"],
    )
    op.drop_constraint(
    ...
    op.create_check_constraint(
        ...
        "venue_fact_input_links",
        _link_fact_check(include_account_equity=True),
    )
    _replace_venue_fact_guards(include_account_equity=True)
    op.execute(
        """
        CREATE TRIGGER venue_account_equity_snapshots_immutable
        BEFORE UPDATE OR DELETE ON venue_account_equity_snapshots
        FOR EACH ROW EXECUTE FUNCTION deny_venue_fact_change()
        """
    )
    op.execute(
        """
        CREATE TRIGGER venue_account_equity_snapshots_insert_guard
        BEFORE INSERT ON venue_account_equity_snapshots
        FOR EACH ROW EXECUTE FUNCTION protect_canonical_venue_fact_insert()
        """
    )
    op.execute(
        """
        CREATE CONSTRAINT TRIGGER venue_account_equity_snapshots_first_link_guard
        AFTER INSERT ON venue_account_equity_snapshots
...
def downgrade() -> None:
    op.execute(
        ...
        DO $$
        BEGIN
            IF EXISTS (SELECT 1 FROM venue_account_equity_snapshots) THEN
                RAISE EXCEPTION
                    'cannot downgrade account-equity snapshots while canonical facts remain';
            END IF;
        END;
        $$
        """
    ...
    op.execute(
        "DROP TRIGGER IF EXISTS venue_account_equity_snapshots_first_link_guard "
        "ON venue_account_equity_snapshots"
    )
    op.execute(
        "DROP TRIGGER IF EXISTS venue_account_equity_snapshots_insert_guard "
        "ON venue_account_equity_snapshots"
    )
    op.execute(
        "DROP TRIGGER IF EXISTS venue_account_equity_snapshots_immutable "
        "ON venue_account_equity_snapshots"
    )
    _replace_venue_fact_guards(include_account_equity=False)
    op.drop_constraint(
        "ck_venue_fact_input_links_exact_fact", "venue_fact_input_links", type_="check"
    )
    op.drop_constraint(
    ...
    op.drop_column("venue_fact_input_links", "venue_account_equity_snapshot_id")
    op.create_check_constraint(
        "ck_venue_fact_input_links_exact_fact",
        "venue_fact_input_links",
        _link_fact_check(include_account_equity=False),
    )
    op.drop_index(
        "ix_venue_account_equity_snapshots_scope_time",
        table_name="venue_account_equity_snapshots",
    )
    op.drop_table("venue_account_equity_snapshots")


def _link_fact_check(*, include_account_equity: bool) -> str:
    account_null = "AND venue_account_equity_snapshot_id IS NULL " if include_account_equity else ""
    account_branch = (
        "OR (source_type = 'VENUE_BALANCES' AND venue_order_observation_id IS NULL "
        ...
        if include_account_equity
        else ""
    )
    return (
        "(source_type = 'VENUE_ORDERS' AND venue_order_observation_id IS NOT NULL "
        "AND venue_fill_id IS NULL AND venue_position_snapshot_id IS NULL "
        f"AND venue_protection_snapshot_id IS NULL {account_null}) OR "
        ...
        "AND venue_fill_id IS NULL AND venue_position_snapshot_id IS NULL "
        f"AND venue_protection_snapshot_id IS NOT NULL {account_null}) "
        f"{account_branch}"
    )


def _replace_venue_fact_guards(*, include_account_equity: bool) -> None:
    account_source = (
        "WHEN 'venue_account_equity_snapshots' THEN 'VENUE_BALANCES'"
        if include_account_equity
        else ""
    )
    op.execute(
        ...
        BEGIN
            expected_source := CASE TG_TABLE_NAME
                ...
                WHEN 'venue_protection_snapshots' THEN 'VENUE_PROTECTION'
                {account_source}
                ELSE NULL
            END;
            ...
            END IF;
            IF TG_TABLE_NAME = 'venue_account_equity_snapshots' AND (
                scope_row.margin_mode <> to_jsonb(NEW) ->> 'margin_mode'
                OR scope_row.collateral_pool_id
                    <> to_jsonb(NEW) ->> 'collateral_pool_id'
            ) THEN
                RAISE EXCEPTION 'canonical venue account equity scope changed';
            END IF;
    ...
    _replace_input_link_guard(include_account_equity=include_account_equity)
    _replace_first_link_guard(include_account_equity=include_account_equity)
    normalized_sources = (
        "'VENUE_ORDERS', 'VENUE_FILLS', 'VENUE_POSITIONS', 'VENUE_BALANCES', 'VENUE_PROTECTION'"
        if include_account_equity
        else "'VENUE_ORDERS', 'VENUE_FILLS', 'VENUE_POSITIONS', 'VENUE_PROTECTION'"
    )
    op.execute(
        f"""
...
def _replace_input_link_guard(*, include_account_equity: bool) -> None:
    account_branch = (
        """
...
def _replace_first_link_guard(*, include_account_equity: bool) -> None:
    account_branch = (
        """
